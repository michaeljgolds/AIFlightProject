...
// Throughout LAC, all global variable definitions
// can always be found in one of two locations as
// follows:
//
// 1 of 2: Most global variables are defined here,
// at the top of "main.cpp".
//
// 2 of 2: All of the remaining global variables are defined
...
bool MissionNetworkBattle01BlueTeamVictory = false;
bool MissionNetworkBattle01RedTeamVictory = false;
bool MissionNetworkBattle02BlueTeamVictory = false;
bool MissionNetworkBattle02RedTeamVictory = false;
bool MissionNetworkBattle03BlueTeamVictory = false;
bool MissionNetworkBattle03RedTeamVictory = false;
bool MissionRunning;
...
unsigned char MissionStateNetworkBattle01;
unsigned char MissionStateNetworkBattle02;
unsigned char MissionStateNetworkBattle03;
unsigned char MorseCharacterIn = 0;
unsigned char MorseCharacterOut = 0;
...
unsigned char PreviousMorseSender = 0;
...
unsigned char ServerMessageRecipient;
...
int NetworkReceiveTimer = 0;
int NetworkReceiveTimerInterval = 100;
...
Uint32 MostRecentBombFlightTimer = 0;
...
float InPacketDeltaXPosition[32];
float InPacketDeltaYPosition[32];
float InPacketDeltaZPosition[32];
float NetDeltaX;
float NetDeltaY;
float NetDeltaZ;
...
// global MapLoader pointer variables:
MapLoader *maploader;
...
int main (int argc, char **argv)
   ...
   GunCamHistoryFile = fopen ("GunCamHistory.LAC", "a");
   if (GunCamHistoryFile == NULL)
      {
      display ("main() failed to open GunCamHistory.LAC", LOG_MOST);
      }
   else
      {
      display ("main() succesfully opened GunCamHistory.LAC", LOG_MOST);
      }

   OnlineScoreLogFile = fopen ("OnlineScoreLog.LAC", "a");
   if (OnlineScoreLogFile == NULL)
      {
      display ("main() failed to open OnlineScoreLogFile.LAC", LOG_MOST);
      }
   else
      {
      display ("main() succesfully opened OnlineScoreLogFile.LAC", LOG_MOST);
      }
...
void callbackAntialiasing (Component *comp, int key)
    {
    display ("Function Entry callbackAntialiasing()", LOG_MOST);
...
void callbackBrightness (Component *comp, int key)
    {
    display ("Function Entry callbackBrightness()", LOG_MOST);
    char buf [256];
...
void callbackControls (Component *comp, int key)
    {
    display ("Function Entry callbackControls()", LOG_MOST);
...
void callbackCredits (Component *comp, int key)
    {
    display ("Function Entry callbackCredits()", LOG_MOST);
    void switch_credits ();
    switch_credits ();
    }
...
void callbackDifficulty (Component *comp, int key)
    {
    display ("Function Entry callbackDificulty()", LOG_MOST);
    char buf [256];
    if (key == MOUSE_BUTTON_LEFT)
        {
        difficulty = 2;
...
void callbackFighter (Component *comp, int key)
    {
    display ("callbackFighter()", LOG_MOST);
...
void callbackFighterNext (Component *comp, int key)
    ...
    if (key == MOUSE_BUTTON_LEFT)
        ...
        display (DebugBuf, LOG_MOST);
        display ("callbackFighterNext() decrementing SelectedAircraft. Now", LOG_MOST);
...
void callbackFighterPrev (Component *comp, int key)
    ...
    if (key == MOUSE_BUTTON_LEFT)
        ...
        display (DebugBuf, LOG_MOST);
        display ("callbackFighterPrev() decrementing SelectedAircraft. Now", LOG_MOST);
        sprintf (DebugBuf, "%d", SelectedAircraft);
        display (DebugBuf, LOG_MOST);
...
void callbackFullscreen (Component *comp, int key)
    ...
    if (wantfullscreen)
        {
        ((Label *) optmenu [0]->components [18])->setText ("YES");
        }
    else
        {
        ((Label *) optmenu [0]->components [18])->setText ("NO");
        }
    }
...
void callbackJoystickAxis (Component *comp, int key)
    ...
    int i;
    int buttonnum = -1;
    display ("callbackJoystickAxis()", LOG_MOST);
...
void callbackKeyboard (Component *comp, int key)
    {
    display ("callbackKeyboard()", LOG_MOST);
    int i;
    ...
    for (i = 5; i < 42; i ++)
        if (comp == controlsmenu [0]->components [i])
            ...
            if      (i == z ++)
                {
                key_PRIMARY = key;
                display ("callbackKeyboard() mapping key_PRIMARY", LOG_MOST);
                ...
                {
                key_SECONDARY = key;
                display ("callbackKeyboard() mapping key_SECONDARY", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                {
                key_DROPFLARE = key;
                display ("callbackKeyboard() mapping key_DROPFLARE", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                {
                key_DROPCHAFF = key;
                display ("callbackKeyboard() mapping key_DROPCHAFF", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                ...
                display ("callbackKeyboard() mapping key_WEAPONSELECT", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                {
                key_TARGETNEXT = key;
                display ("callbackKeyboard() mapping key_TARGETNEXT", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                ...
                display ("callbackKeyboard() mapping key_TARGETPREVIOUS", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                ...
                display ("callbackKeyboard() mapping key_ToggleViewIntExt", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                {
                key_FlapsUp = key;
                display ("callbackKeyboard() mapping key_FlapsUp", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                {
                key_FlapsDn = key;
                display ("callbackKeyboard() mapping key_FlapsDn", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                ...
                display ("callbackKeyboard() mapping key_GunCameraToggle", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                ...
                display ("callbackKeyboard() mapping key_WepAfterburner", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                {
                key_SpeedBrake = key;
                display ("callbackKeyboard() mapping key_SpeedBrake", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                ...
                display ("callbackKeyboard() mapping key_Undercarriage", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                {
                key_RadarZoomIn = key;
                display ("callbackKeyboard() mapping key_RadarZoomIn", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                ...
                display ("callbackKeyboard() mapping key_RadarZoomOut", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                {
                key_HudOnOff = key;
                display ("callbackKeyboard() mapping key_HudOnOff", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                {
                key_IffOnOff = key;
                display ("callbackKeyboard() mapping key_IffOnOff", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                ...
                display ("callbackKeyboard() mapping key_HudLadderBarsOnOff", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                ...
                display ("callbackKeyboard() mapping key_MapViewOnOff", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                {
                key_MapZoomIn = key;
                display ("callbackKeyboard() mapping key_MapZoomIn", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                {
                key_MapZoomOut = key;
                display ("callbackKeyboard() mapping key_MapZoomOut", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                ...
                display ("callbackKeyboard() mapping key_MapScrollEast", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                ...
                display ("callbackKeyboard() mapping key_MapScrollWest", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                ...
                display ("callbackKeyboard() mapping key_MapScrollNorth", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                ...
                display ("callbackKeyboard() mapping key_MapScrollSouth", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                ...
                display ("callbackKeyboard() mapping key_TARGETVOCALIZE", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                {
                key_MumblePtt = key;
                display ("callbackKeyboard() mapping key_MumblePtt", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                ...
                display ("callbackKeyboard() mapping key_MumbleWhisper", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                {
                key_ZoomFovIn = key;
                display ("callbackKeyboard() key_ZoomFovIn", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                {
                key_ZoomFovOut = key;
                display ("callbackKeyboard() key_ZoomFovOut", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            else if (i == z ++)
                {
                key_RadarOnOff = key;
                display ("callbackKeyboard() key_RadarOnOff", LOG_MOST);
                sprintf (DebugBuf, "Key = %d", key);
                display (DebugBuf, LOG_MOST);
                }
            }
...
void callbackMainMenu (Component *comp, int key)
    {
    display ("Function entry: callbackMainMenu()", LOG_MOST);
    int i;
...
void callbackMouseAutorudder (Component *comp, int key)
    {
    display ("Function entry: callbackMouseAutoRudder()", LOG_MOST);
    char buf [256];
...
void callbackMouseDefaults (Component *comp, int key)
    {
    display ("Function entry: callbackMouseDefaults()", LOG_MOST);
    char buf [256];
...
void callbackMouseRelative (Component *comp, int key)
    {
    display ("Function entry: callbackMouseRelative()", LOG_MOST);
    char buf [256];
...
void callbackMouseReverse (Component *comp, int key)
    {
    display ("Function entry: callbackMouseReverse()", LOG_MOST);
    char buf [256];
...
void callbackMouseSensitivity (Component *comp, int key)
    {
    display ("Function entry: callbackMouseSensitivity()", LOG_MOST);
    char buf [256];
...
void callbackMusic (Component *comp, int key)
    {
    display ("Function entry: callbackMusic()", LOG_MOST);
    void playRandomMusic ();
    void ReshapeMenu ();
    char buf [256];
...
void callbackPhysics (Component *comp, int key)
    {
    display ("Function entry: callbackPhysics()", LOG_MOST);
    char buf [256];
    physics = 1;
...
void callbackQuality (Component *comp, int key)
    {
    display ("Function entry: callbackQuality()", LOG_MOST);
    char buf [256];
...
void callbackQuit (Component *comp, int key)
    {
    display ("Function entry: callbackQuit()", LOG_MOST);
    void switch_quit ();
    switch_quit ();
    }
...
void callbackQuitNow (Component *comp, int key)
    {
    display ("Function entry: callbackNow()", LOG_MOST);
    void game_quit ();
    game_quit ();
    }
...
void callbackResolution (Component *comp, int key)
    {
    display ("Function entry: callbackResolution()", LOG_MOST);
    const int numres = 5;
...
void callbackReturn (Component *comp, int key)
    {
    display ("Function entry: callbackReturn()", LOG_MOST);
    void switch_game ();
    switch_game ();
    }
...
void callbackSound (Component *comp, int key)
    {
    display ("Function entry: callbackSound()", LOG_MOST);
    void ReshapeMenu ();
    void setPlaneVolume ();
    char buf [256];
...
void callbackSpecialEffects (Component *comp, int key)
    {
    display ("Function entry: callbackSpecialEffects()", LOG_MOST);
    char buf [5];
...
void callbackSwitchMainMenu (Component *comp, int key)
    {
    display ("Function entry: callbackSwitchMainMenu()", LOG_MOST);
    void switch_menu ();
    switch_menu ();
    }
...
void callbackSwitchStartMission (Component *comp, int key)
    {
    display ("Function entry: callbackSwitchStartMission()", LOG_MOST);
    int game_levelInit ();
    void pleaseWait ();
    void switch_game ();
    void switch_menu ();
    ...
    if (MissionNumber == 1)
        ...
        {
        sound->loadMusic (MUSIC_TUTORIAL04);
        sound->playMusic (1);
        }
    else if (MissionNumber == 5)
        {
        sound->loadMusic (MUSIC_TURKEYSHOOT);
        sound->playMusic (1);
        }
    else if (MissionNumber == 6)
        {
        sound->loadMusic (MUSIC_EVERYMANFORHIMSELF);
        sound->playMusic (1);
        }
    else if (MissionNumber == 7)
        {
        sound->loadMusic (MUSIC_TEAMSMISSILESANDGUNS);
        sound->playMusic (1);
        }
    else if (MissionNumber == 8)
        {
        sound->loadMusic (MUSIC_FREEFLIGHTWW2);
        sound->playMusic (1);
        }
    else if (MissionNumber == 9)
        {
        sound->loadMusic (MUSIC_TEAMBASE);
        sound->playMusic (1);
        }
    else if (MissionNumber == 10)
        {
        sound->loadMusic (MUSIC_WAVES1);
        sound->playMusic (1);
        }
    else if (MissionNumber == 11)
        {
        sound->loadMusic (MUSIC_JETBAIT);
        sound->playMusic (1);
        }
    else if (MissionNumber == 12)
        {
        display ("Loading music file for MissionHeadToHead00.", LOG_MOST);
        ...
        sound->playMusic (1);
        display ("Playing music for MissionNetworkBattle03.", LOG_MOST);
        }
    else
        {
        sound->loadMusic (MUSIC_MUSIC02ANDFURBALLAA);
...
void callbackTraining (Component *comp, int key)
    {
    display ("Function Entry callbackTraining()", LOG_MOST);
...
void callbackView (Component *comp, int key)
    {
    display ("Function entry: callbackView()", LOG_MOST);
    void ReshapeMenu ();
    char buf [256];
...
CModel *getModel (int id)
    ...
    else if (id == FLARAK_AIR1)
        {
        return &model_flarak1;
        }
    else if (id == STATIC_HALL1)
        {
        return &model_hall1;
        }
...
void config_test (int argc, char **argv)
    ...
    if (valids == -1)
        ...
        display ((char *)"Online help is available at AskMisterWizard.com/FlightSimMovies/LAC", LOG_FATAL);
        exit (EXIT_INIT);
        }
...
void createMenu ()
    {
    display ("Function entry: createMenu()", LOG_MOST);
    ...
    */
    ...
    strncpy (submenu1names [z ++], "TUTORIAL4: AERODYNAMICS", 24);
    strncpy (submenu1names [z ++], "TURKEY SHOOT", 13);
    strncpy (submenu1names [z ++], "EVERY MAN FOR HIMSELF", 22);
    strncpy (submenu1names [z ++], "TEAMS, MISSILES, AND GUNS", 26);
    strncpy (submenu1names [z ++], "FREE FLIGHT WW2", 16);
    strncpy (submenu1names [z ++], "BASE VERSUS BASE", 17);
    strncpy (submenu1names [z ++], "ASSAULT WAVES", 14);
    strncpy (submenu1names [z ++], "", 2);
    ...
    yf = ysubmenu;
    yfstep = 1;
    button = new Button ("DEFAULT AIRCRAFT");
    ...
    */
    ...
    label = new Label ("REMAP KEYBOARD KEYS");
    ...
    yf = yf + (5 * yfstep);
    z = 0;
    display ("Register keyboard control submenu", LOG_MOST);
    ...
    */
    ...
    xfstep = 12.5;
    yfstep = 0.8;
    button = new Button ("JOYSTICK AILERON");
    ...
    yf -= yfstep;
    button = new Button ("JOYSTICK ELEVATOR");
    button->setFunction (callbackJoystickAxis);
    button->setBounds (xf, yf, xfstep, yfstep - 0.1);
    controlsmenu [2]->add (button);
    ...
    controlsmenu [2]->add (label);
    yf -= yfstep;
    button = new Button ("JOYSTICK/THROTTLE");
    button->setFunction (callbackJoystickAxis);
    button->setBounds (xf, yf, xfstep, yfstep - 0.1);
    controlsmenu [2]->add (button);
    ...
    controlsmenu [2]->add (label);
    yf -= yfstep;
    button = new Button ("JOYSTICK/RUDDER");
    button->setFunction (callbackJoystickAxis);
    button->setBounds (xf, yf, xfstep, yfstep - 0.1);
    controlsmenu [2]->add (button);
    ...
    controlsmenu [2]->add (label);
    yf -= yfstep;
    button = new Button ("JOYSTICK VIEW X");
    button->setFunction (callbackJoystickAxis);
    button->setBounds (xf, yf, xfstep, yfstep - 0.1);
    controlsmenu [2]->add (button);
    ...
    controlsmenu [2]->add (label);
    yf -= yfstep;
    button = new Button ("JOYSTICK VIEW Y");
    button->setFunction (callbackJoystickAxis);
    button->setBounds (xf, yf, xfstep, yfstep - 0.1);
    controlsmenu [2]->add (button);
    ...
    controlsmenu [2]->add (label);
    yf-=yfstep;
    label = new Label ("TO MAP JOYSTICK AXES, CLICK ON THE");
    label->setBounds (xf-14, yf, xfstep, yfstep);
    controlsmenu [2]->add (label);
    yf-=yfstep;
    label = new Label ("JOYSTICK AXIS BUTTONS ABOVE.");
    label->setBounds (xf-14, yf, xfstep, yfstep);
    controlsmenu [2]->add (label);
    yf-=yfstep;
    label = new Label ("");
    ...
    controlsmenu [2]->add (label);
    yf-=yfstep;
    label = new Label ("TO MAP JOYSTICK BUTTONS, USE YOUR");
    label->setBounds (xf-14, yf, xfstep, yfstep);
    controlsmenu [2]->add (label);
    yf-=yfstep;
    label = new Label ("FAVORITE TEXT EDITOR TO EDIT THE");
    label->setBounds (xf-14, yf, xfstep, yfstep);
    controlsmenu [2]->add (label);
    yf-=yfstep;
    label = new Label ("LACCONTROLS.TXT FILE IN THE .LAC");
    label->setBounds (xf-14, yf, xfstep, yfstep);
    controlsmenu [2]->add (label);
    yf-=yfstep;
    label = new Label ("FOLDER WITHIN YOUR HOME DIRECTORY.");
    label->setBounds (xf-14, yf, xfstep, yfstep);
    controlsmenu [2]->add (label);
    yf-=yfstep;
    label = new Label ("");
    ...
    controlsmenu [2]->add (label);
    yf-=yfstep;
    label = new Label ("TO MAP KEYBOARD KEYS, CLICK THE");
    label->setBounds (xf-14, yf, xfstep, yfstep);
    controlsmenu [2]->add (label);
    yf-=yfstep;
    label = new Label ("CONTROLS BUTTON ABOVE, WHERE IT SAYS");
    label->setBounds (xf-14, yf, xfstep, yfstep);
    controlsmenu [2]->add (label);
    yf-=yfstep;
    label = new Label ("NEXT PAGE.");
    label->setBounds (xf-14, yf, xfstep, yfstep);
    controlsmenu [2]->add (label);
    yf-=yfstep;
    label = new Label ("");
    ...
    */
    ...
    textfield = new TextField ("    PREV/NEXT AIRCRAFT   ", yfstep - 0.1);
...
void createMission (int missionid)
    ...
    if (missionid == MISSION_DEMO)
    ...
    else if (missionid == MISSION_TUTORIAL4)
        {

        MissionNumber = 4;
        missionnew = new MissionTutorial4 ();
        }
    else if (missionid == MISSION_TURKEYSHOOT)
        {
        MissionNumber = 5;
        missionnew = new MissionTurkeyShoot ();
        }
    else if (missionid == MISSION_EVERYMANFORHIMSELF)
        {
        MissionNumber = 6;
        missionnew = new MissionEveryManForHimself ();
        }
    else if (missionid == MISSION_TEAMSMISSILESANDGUNS)
        {
        MissionNumber = 7;
        missionnew = new MissionTeamsMissilesAndGuns ();
        }
    ...
    else if (missionid == MISSION_TEAMBASE1)
        {
        MissionNumber = 9;
        missionnew = new MissionTeamBase1 ();
        }
    else if (missionid == MISSION_WAVES1)
        {
        MissionNumber = 10;
        missionnew = new MissionWaves1 ();
        }
    else if (missionid == MISSION_JETBAIT1)
        {
        MissionNumber = 11;
        missionnew = new MissionJetbait1 ();
        }
        ...
        {
        MissionNumber = 13;
        display ("Setting missionnew to MissionNetworkBattle01", LOG_MOST);
        ...
        {
        MissionNumber = 14;
        display ("Setting missionnew to MissionNetworkBattle02", LOG_MOST);
        ...
        {
        MissionNumber = 15;
        display ("Setting missionnew to MissionNetworkBattle03", LOG_MOST);
...
void DisplayCredits ()
    ...
    float fontzoom = 1;
    font1->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "LINUX AIR COMBAT WAS WRITTEN BY", col2);
    font1->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "ROBERT BOSEN, USING A FRAMEWORK", col2);
    font1->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "DERIVING FROM GL-117 BY TOM DREXL.", col2);
    font1->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "NO MICROSOFT PRODUCTS WERE USED", col);
    font1->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "IN THE PRODUCTION OF THIS SOFTWARE.", col);
    font1->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "NO APPLE PRODUCTS WERE USED", col);
    font1->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "IN THE PRODUCTION OF THIS SOFTWARE.", col);
    font1->zoom = 0.08;
    font2->zoom = 0.06;
    fontzoom = 0.8;
    font2->drawTextCentered (0, (yt -= 4 * ydist) / fontzoom, zf, "NETWORK DEVELOPMENT SUPPORT: ADAM BOSEN", col);
    font2->drawTextCentered (0, (yt -= 4 * ydist) / fontzoom, zf, "IL2 AUTOGUNNER CODE: HYRUM BOSEN", col);
    font2->drawTextCentered (0, (yt -= 4 * ydist) / fontzoom, zf, "GRAPHICS SUPPORT: STEFANO PERIS, BLAKE WILLIAMS, TY WILLIAMS", col);
    font2->drawTextCentered (0, (yt -= 4 * ydist) / fontzoom, zf, "ALPHA TESTERS: TY WILLIAMS, BLAKE WILLIAMS, HYRUM BOSEN", col);
    font2->drawTextCentered (0, (yt -= 4 * ydist) / fontzoom, zf, "PUBLIC-DOMAIN MUSIC CLIPS: ALEXANDER NAKARADA", col);

    font2->drawTextCentered (0, (yt -= 4 * ydist) / fontzoom, zf, "DEVELOPMENT OF LINUX AIR COMBAT WAS GIVEN A", col);
    font2->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "TREMENDOUS HEAD START BY AN AMAZING SET OF FREE", col);
    font2->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "SOURCE CODE, OPERATING SYSTEM TOOLS, LANGUAGES,", col);
    font2->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "AND TOOLS. IN ADDITION TO THE LINUX OPERATING SYSTEM", col);
    font2->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "BY LINUS TORVALDS AND GNU C AND CLIBS BY RICHARD", col);
    font2->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "STALLMAN AND THE GNU PROJECT, SPECIAL THANKS GO TO MANY", col);
    font2->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "OTHER PREDECESSORS IN THE GL-117 PROJECT,", col);
    font2->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "INCLUDING:", col);
    font2->drawTextCentered (0, (yt -= 4 * ydist) / fontzoom, zf, "TECHNICAL UNIVERSITY OF MUNICH, THOMAS A. DREXL, PIOTER", col);
    font2->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "PAWLOW, WOLFGANG HOMMEL, LOURENS VEEN, ARNE REINERS", col);
    font2->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "AND COUNTLESS OTHERS WHO CREATED THE UNDERLYING LINUX", col);
    font2->drawTextCentered (0, (yt -= 2 * ydist) / fontzoom, zf, "GRAPHIC AND MEDIA LIBRARIES OPENGL, GLUT, SDL, ETC.", col);
    font2->zoom = 0.1;
    font2->zoom = 0.1;
    glPopMatrix ();
    }
...
void DisplayFighter ()
    ...
    if (ffighter.id == FIGHTER_P38L)
       ...
       {
       yf -= 1;
       strncpy (buf, "SPEED: 650   CLIMB: 6500 FT/MIN", 32);
       if (ffighter.HistoricPeriod == 1)
           {
           strncat (buf, "     EARLY WW2", 15);
           }
       ...
       {
       yf -= 1;
       strncpy (buf, "SPEED: 287   CLIMB: 1300 FT/MIN", 32);
       if (ffighter.HistoricPeriod == 1)
           {
           strncat (buf, "     EARLY WW2", 15);
           }
       ...
       {
       yf -= 1;
       strncpy (buf, "SPEED: 331   CLIMB: 2200 FT/MIN", 31);
       if (ffighter.HistoricPeriod == 1)
           {
           strncat (buf, "     EARLY WW2", 15);
           }
       ...
       {
       yf -= 1;
       strncpy (buf, "SPEED: 255   CLIMB: 800 FT/MIN", 31);
       if (ffighter.HistoricPeriod == 1)
           {
           strncat (buf, "     EARLY WW2", 15);
           }
       ...
       {
       yf -= 1;
       strncpy (buf, "SPEED: 272   CLIMB: 980 FT/MIN", 31);
       if (ffighter.HistoricPeriod == 1)
           {
           strncat (buf, "     EARLY WW2", 15);
           }
       ...
       {
       yf -= 1;
       strncpy (buf, "SPEED: 300   CLIMB: 1250 FT/MIN", 32);
       if (ffighter.HistoricPeriod == 1)
           {
           strncat (buf, "     EARLY WW2", 15);
           }
       ...
       {
       yf -= 1;
       strncpy (buf, "SPEED: 291   CLIMB: 2900 FT/MIN", 32);
       if (ffighter.HistoricPeriod == 1)
           {
           strncat (buf, "     EARLY WW2", 15);
           }
       ...
       {
       yf -= 1;
       strncpy (buf, "SPEED: 290   CLIMB: 1025 FT/MIN", 32);
       if (ffighter.HistoricPeriod == 1)
           {
           strncat (buf, "     EARLY WW2", 15);
           }
       ...
       {
       yf -= 1;
       strncpy (buf, "SPEED: 282   CLIMB: 720 FT/MIN", 31);
       if (ffighter.HistoricPeriod == 1)
           {
           strncat (buf, "     EARLY WW2", 15);
           }
       ...
       {
       yf -= 1;
       strncpy (buf, "SPEED: 412   CLIMB: 274O FT/MIN", 31);
       if (ffighter.HistoricPeriod == 1)
           {
           strncat (buf, "     EARLY WW2", 15);
           }
       ...
       font1->drawText (textx / fontzoom, yf / fontzoom, -2, buf, &menu_colwhite);
       yf -= 1;
       strncpy (buf, "ARMAMENT: 1X20MM, 2X30CAL", 26);
       font1->drawText (textx / fontzoom, yf / fontzoom, -2, buf, &menu_colwhite);
       ...
       {
       display ("DisplayFighter() cannot provide details for undefined aircraft.", LOG_MOST);
       DefaultAircraft = FIGHTER_P38L;
       }
    font1->zoom = 0.1;
    drawMouseCursor ();
...
// SunBlinding test
    ...
    if (camera !=9)
       ...
       static Uint32 PriorBombTimer;
       float CCIPoffsetX;
       float CCIPoffsetY;
       float DegreesToRadiansFactor = -0.01745333;

       if (MostRecentBombFlightTimer > PriorBombTimer)
          {

          }
       if (PriorBombTimer == MostRecentBombFlightTimer)
          {
          //display ("LookMap() has no bomb in flight.", LOG_MOST);
          }
       if ((fplayer->missiletype == 0) && (fplayer->missiles [fplayer->missiletype] && MapZoomFactor == 0))
          ...
          CCIPoffsetX = sin(fplayer->phi * DegreesToRadiansFactor) * fplayer->realspeed;
          CCIPoffsetX *= fplayer->tl->y;
          CCIPoffsetX /= 1.9;
          CCIPoffsetY = cos(fplayer->phi * DegreesToRadiansFactor) * fplayer->realspeed;
          CCIPoffsetY *= fplayer->tl->y;
          CCIPoffsetY /= 1.9;
          sprintf (buf, "O"); // White "O" marks player position. Green "O" marks CCIP.
          if (MapEastWestScroll == 0 && MapNorthSouthScroll == 0)
             {
       ...
       PriorBombTimer = MostRecentBombFlightTimer;
       }
    bool write = false;
...
void event_AutoPilot ()
   {
   display ("event_AutoPilot", LOG_MOST);
   if (!AutoPilotActive)
      {
...
void event_CommsMacro01 ()
   {
   display ("event_CommsMacro01", LOG_MOST);
   ...
   if (!strcmp (&CommsMacro01String1[0], "CHANNEL-M1-ALL"))
      {
      display ("event_CommsMacro01 setting MumbleChannelPresumed.", LOG_MOST);
      MumbleChannelPresumed = 1;
      }
   else
      {
      display ("event_CommsMacro01 NOT setting MumbleChannelPresumed because CommsMacro01 has been customized.", LOG_MOST);
      return;
      }
...
void event_CommsMacro02 ()
   {
   display ("event_CommsMacro02", LOG_MOST);
   ...
   if (!strcmp (&CommsMacro02String1[0], "CHANNEL-M1-BLUE"))
      {
      display ("event_CommsMacro02 setting MumbleChannelPresumed.", LOG_MOST);
      MumbleChannelPresumed = 2;
      }
   else
      {
      display ("event_CommsMacro02 NOT setting MumbleChannelPresumed because CommsMacro02 has been customized.", LOG_MOST);
      return;
      }
   ...
   if (CurrentMissionNumber == MISSION_NETWORKBATTLE02 || CurrentMissionNumber == MISSION_NETWORKBATTLE03)
      ...
      if (MyNetworkId%2 == 0)
         ...
         if (ThreeDObjects[29]->Durability >= ThreeDObjects[29]->maxDurability)
            {
            display ("event_CommsMacro02() Radio announcing Enemy HQ Airfield Status 100%", LOG_MOST);
            ...
            if (myrandom(100)%2)
               ...
               if (AirfieldRepairsAcceleratedForRedTeam)
                  {
                  display ("event_CommsMacro08() Radio announcing Enemy HQ Airfield repairs accelerated", LOG_MOST);
                  sound->play (SOUND_ENEMYHQREPAIRSPROCEEDINGRAPIDLY, false);
                  }
               else if (AirfieldRepairsStoppedForRedTeam)
                  {
                  display ("event_CommsMacro08() Radio announcing Enemy HQ Airfield repairs stopped", LOG_MOST);
                  sound->play (SOUND_ENEMYHQREPAIRSSTOPPED, false);
                  }
               else if (AirfieldRepairRateNormalForRedTeam)
                  {
                  display ("event_CommsMacro08() Radio announcing Enemy HQ Airfield repairs proceeding normally.", LOG_MOST);
...
void event_CommsMacro03 ()
   {
   display ("event_CommsMacro03", LOG_MOST);
   ...
   if (!strcmp (&CommsMacro03String1[0], "CHANNEL-M1-RED"))
      {
      display ("event_CommsMacro03 setting MumbleChannelPresumed.", LOG_MOST);
      MumbleChannelPresumed = 3;
      }
   else
      {
      display ("event_CommsMacro03 NOT setting MumbleChannelPresumed because CommsMacro03 has been customized.", LOG_MOST);
      return;
      }
   ...
   if (CurrentMissionNumber == MISSION_NETWORKBATTLE02 || CurrentMissionNumber == MISSION_NETWORKBATTLE03)
      ...
      if (MyNetworkId%2)
         ...
         if (ThreeDObjects[28]->Durability >= ThreeDObjects[28]->maxDurability)
            {
            display ("event_CommsMacro03() Radio announcing Enemy HQ Airfield Status 100%", LOG_MOST);
            sound->setVolume (SOUND_ENEMYHQSTATUS100A, 127);
            sound->play (SOUND_ENEMYHQSTATUS100A, false);
            }
         else
...
void event_CommsMacro04 ()
   {
   display ("event_CommsMacro04", LOG_MOST);
   ...
   if (!strcmp (&CommsMacro04String1[0], "CHANNEL-M2-ALL"))
      {
      display ("event_CommsMacro04 setting MumbleChannelPresumed.", LOG_MOST);
      MumbleChannelPresumed = 4;
      }
   else
      {
      display ("event_CommsMacro04 NOT setting MumbleChannelPresumed because CommsMacro04 has been customized.", LOG_MOST);
      return;
      }
...
void event_CommsMacro05 ()
   {
   display ("event_CommsMacro05", LOG_MOST);
   ...
   if (!strcmp (&CommsMacro05String1[0], "CHANNEL-M2-BLUE"))
      {
      display ("event_CommsMacro05 setting MumbleChannelPresumed.", LOG_MOST);
      MumbleChannelPresumed = 5;
      }
   else
      {
      display ("event_CommsMacro05 NOT setting MumbleChannelPresumed because CommsMacro05 has been customized.", LOG_MOST);
      return;
      }
   ...
   if (CurrentMissionNumber == MISSION_NETWORKBATTLE01 || CurrentMissionNumber == MISSION_NETWORKBATTLE03)
      ...
      if (MyNetworkId%2 == 0)
         ...
         if (ThreeDObjects[29]->Durability >= ThreeDObjects[29]->maxDurability)
            {
            display ("event_CommsMacro05() Radio announcing Enemy HQ Airfield Status 100%", LOG_MOST);
            sound->setVolume (SOUND_ENEMYHQSTATUS100A, 127);
            sound->play (SOUND_ENEMYHQSTATUS100A, false);
            }
         else
...
void event_CommsMacro06 ()
   {
   display ("event_CommsMacro06", LOG_MOST);
   ...
   if (!strcmp (&CommsMacro06String1[0], "CHANNEL-M2-RED"))
      {
      display ("event_CommsMacro06 setting MumbleChannelPresumed.", LOG_MOST);
      MumbleChannelPresumed = 6;
      }
   else
      {
      display ("event_CommsMacro06 NOT setting MumbleChannelPresumed because CommsMacro06 has been customized.", LOG_MOST);
      return;
      }
   ...
   if (CurrentMissionNumber == MISSION_NETWORKBATTLE01 || CurrentMissionNumber == MISSION_NETWORKBATTLE03)
      ...
      if (MyNetworkId%2)
         ...
         if (ThreeDObjects[28]->Durability >= ThreeDObjects[28]->maxDurability)
            {
            display ("event_CommsMacro06() Radio announcing Enemy HQ Airfield Status 100%", LOG_MOST);
            sound->setVolume (SOUND_ENEMYHQSTATUS100A, 127);
            sound->play (SOUND_ENEMYHQSTATUS100A, false);
            }
         else
...
void event_CommsMacro07 ()
   {
   display ("event_CommsMacro07", LOG_MOST);
   ...
   if (!strcmp (&CommsMacro07String1[0], "CHANNEL-M3-ALL"))
      {
      display ("event_CommsMacro07 setting MumbleChannelPresumed.", LOG_MOST);
      MumbleChannelPresumed = 7;
      }
   else
      {
      display ("event_CommsMacro07 NOT setting MumbleChannelPresumed because CommsMacro07 has been customized.", LOG_MOST);
      return;
      }

   MumbleCurrentMissionBlueSubChannelRequested = false;
   MumbleCurrentMissionRedSubChannelRequested = false;
   display ("event_CommsMacro07() CurrentMissionNumber = ", LOG_MOST);
   ...
   if (CurrentMissionNumber == MISSION_NETWORKBATTLE01 || CurrentMissionNumber == MISSION_NETWORKBATTLE02)
      {
      MumbleChannelExitRequested = true;
      display ("event_CommsMacro07() MumbleChannelExitRequested = true", LOG_MOST);
      MumbleCurrentMissionChannelRequested = false;
      MumbleTunedToCurrentMissionChannel = false;
      display ("event_CommsMacro07() MumbleCurrentMissionChannelRequested = false", LOG_MOST);
      }
   else
      {
      MumbleChannelExitRequested = false;
      display ("event_CommsMacro07() MumbleChannelExitRequested = false", LOG_MOST);
      MumbleCurrentMissionChannelRequested = true;
      MumbleTunedToCurrentMissionChannel = true;
      display ("event_CommsMacro07() MumbleCurrentMissionChannelRequested = true", LOG_MOST);
      }
   }

void event_CommsMacro08 ()
   {
   display ("event_CommsMacro08", LOG_MOST);
   ...
   if (!strcmp (&CommsMacro08String1[0], "CHANNEL-M3-BLUE"))
      {
      display ("event_CommsMacro08 setting MumbleChannelPresumed.", LOG_MOST);
      MumbleChannelPresumed = 8;
      }
   else
      {
      display ("event_CommsMacro08 NOT setting MumbleChannelPresumed because CommsMacro08 has been customized.", LOG_MOST);
      return;
      }
   ...
   if (CurrentMissionNumber == MISSION_NETWORKBATTLE01 || CurrentMissionNumber == MISSION_NETWORKBATTLE02)
      ...
      if (MyNetworkId%2 == 0)
         ...
         if (ThreeDObjects[29]->Durability >= ThreeDObjects[29]->maxDurability)
            {
            display ("event_CommsMacro08() Radio announcing Enemy HQ Airfield Status 100%", LOG_MOST);
            sound->setVolume (SOUND_ENEMYHQSTATUS100A, 127);
            sound->play (SOUND_ENEMYHQSTATUS100A, false);
            }
         else
...
void event_CommsMacro09 ()
   {
   display ("event_CommsMacro09", LOG_MOST);
   ...
   if (!strcmp (&CommsMacro09String1[0], "CHANNEL-M3-RED"))
      {
      display ("event_CommsMacro09 setting MumbleChannelPresumed.", LOG_MOST);
      MumbleChannelPresumed = 9;
      }
   else
      {
      display ("event_CommsMacro09 NOT setting MumbleChannelPresumed because CommsMacro09 has been customized.", LOG_MOST);
      return;
      }
   ...
   if (CurrentMissionNumber == MISSION_NETWORKBATTLE01 || CurrentMissionNumber == MISSION_NETWORKBATTLE02)
      ...
      if (MyNetworkId%2)
         ...
         if (ThreeDObjects[28]->Durability >= ThreeDObjects[28]->maxDurability)
            {
            display ("event_CommsMacro09() Radio announcing Enemy HQ Airfield Status 100%", LOG_MOST);
            sound->setVolume (SOUND_ENEMYHQSTATUS100A, 127);
            sound->play (SOUND_ENEMYHQSTATUS100A, false);
            }
         else
...
void event_CommsMacro10 ()
   {
   display ("event_CommsMacro10", LOG_MOST);
   ...
   if (!strcmp (&CommsMacro10String1[0], "CHANNEL-ROOT"))
      {
      display ("event_CommsMacro10 setting MumbleChannelPresumed.", LOG_MOST);
      MumbleChannelPresumed = 10;
      }
   else
      {
      display ("event_CommsMacro10 NOT setting MumbleChannelPresumed because CommsMacro10 has been customized.", LOG_MOST);
      return;
      }
...
void event_CommsMacro11 ()
   {
   display ("event_CommsMacro11", LOG_MOST);
...
void event_CommsMacro12 ()
   {
   display ("event_CommsMacro012", LOG_MOST);
...
void event_CommsMacro13 ()
   {
   display ("event_CommsMacro13", LOG_MOST);
...
void event_CommsMacro14 ()
   {
   display ("event_CommsMacro14", LOG_MOST);
...
void event_CommsMacro15 ()
   {
   display ("event_CommsMacro15", LOG_MOST);
...
void event_CommsMacro16 ()
   {
   display ("event_CommsMacro16", LOG_MOST);
...
void event_GunCameraToggle()
   ...
   if (CurrentMissionNumber == MISSION_NETWORKBATTLE01 || CurrentMissionNumber == MISSION_NETWORKBATTLE02 || CurrentMissionNumber == MISSION_NETWORKBATTLE03)
      ...
      if (GunCameraActive)
         ...
         sound->setVolume (SOUND_BEEP1, 20);
         sound->play (SOUND_BEEP1, false);
         display ("event_GunCameraToggle() Switching Gun Camera off.", LOG_MOST);
         ...
         sound->play (SOUND_BEEP2, false);
         display ("event_GunCameraToggle() Switching Gun Camera on.", LOG_MOST);
...
void event_targetLocking ()
    ...
    fplayer->targetLockingEnemy ((AIObj **) ThreeDObjects);
    sound->play (SOUND_CLICK1, false);
    }
...
void event_targetNearest ()
    ...
    fplayer->targetNearestEnemy ((AIObj **) ThreeDObjects);
    sound->play (SOUND_CLICK1, false);
    }
...
void event_targetPrevious ()
    ...
    if (InitialTarget == FinalTarget)
       ...
       for (MissionAircraft = 0; MissionAircraft < maxfighter; MissionAircraft++)
          {
          if ((ThreeDObjects[MissionAircraft] == fplayer->target) && (ThreeDObjects[MissionAircraft]->Sentient > 3))
             {
...
void event_targetNext ()
    ...
    if (InitialTarget == FinalTarget)
       ...
       if (CurrentMissionNumber == MISSION_NETWORKBATTLE01 || CurrentMissionNumber == MISSION_NETWORKBATTLE02 || CurrentMissionNumber == MISSION_NETWORKBATTLE03)
          ...
          if (fplayer->target == ThreeDObjects[24])
             {
             display ("event_targetNext() attempted to target object 24. Skipping ahead.", LOG_MOST);
             fplayer->targetNext ((AIObj **) ThreeDObjects);
             fplayer->targetNext ((AIObj **) ThreeDObjects);
             }
          if (fplayer->target == ThreeDObjects[25])
             {
             display ("event_targetNext() attempted to target object 25. Skipping ahead.", LOG_MOST);
             fplayer->targetNext ((AIObj **) ThreeDObjects);
             fplayer->targetNext ((AIObj **) ThreeDObjects);
             }
          if (fplayer->target == ThreeDObjects[26])
             {
             display ("event_targetNext() attempted to target object 26. Skipping ahead.", LOG_MOST);
             fplayer->targetNext ((AIObj **) ThreeDObjects);
             fplayer->targetNext ((AIObj **) ThreeDObjects);
             }
          else if (fplayer->target == ThreeDObjects[27])
             {
             display ("event_targetNext() attempted to target object 27. Skipping ahead.", LOG_MOST);
             fplayer->targetNext ((AIObj **) ThreeDObjects);
             }
...
void game_joystickaxis (int x, int y, int rudder, int throttle,  int JoyHatX, int JoyHatY)
    ...
    if (JoyHatX != PriorJoyHatX)
       ...
       if (JoyHatX > 8000)
          {
          display ("game_joystickaxis() Hat Right", LOG_MOST);
          LacJoystickHatFunc (2);
          }
       else if (JoyHatX < -8000)
          {
          display ("game_joystickaxis() Hat Left", LOG_MOST);
    ...
    if (JoyHatY != PriorJoyHatY)
       ...
       if (JoyHatY > 8000)
          {
          display ("game_joystickaxis() Hat Down", LOG_MOST);
          LacJoystickHatFunc (4);
          }
       else if (JoyHatY < -8000)
          {
          display ("game_joystickaxis() Hat Up", LOG_MOST);
...
int game_levelInit ()
    ...
    if (!MultiPlayer || isserver || !isserver)
        {
    ...
    for (i = 0; i < maxfighter; i ++)
        {
        if (ThreeDObjects [i]->id >= FIGHTER1 && ThreeDObjects [i]->id <= FIGHTER2)
            for (i2 = 0; i2 < 4; i2 ++)
                {
                int type = ThreeDObjects [i]->missilerack [i2];
                CModel *rm = getModel (BOMB01 + type);
                ThreeDObjects [i]->refmodel [i2 * 3] = rm;
                ThreeDObjects [i]->refmodel [i2 * 3 + 1] = rm;
                ThreeDObjects [i]->refmodel [i2 * 3 + 2] = rm;
                ThreeDObjects [i]->reftl [i2 * 3 + 1].z = ThreeDObjects [i]->reftl [i2 * 3].z - 0.04;
                ThreeDObjects [i]->reftl [i2 * 3 + 2].z = ThreeDObjects [i]->reftl [i2 * 3].z + 0.04;
                ThreeDObjects [i]->reftl [i2 * 3].y = ThreeDObjects [i]->reftl [i2 * 3 + 1].y - 0.04;
                int tmp = ThreeDObjects [i]->missilerackn [i2];
                ThreeDObjects [i]->refscale [i2 * 3] = 0.25;
                ThreeDObjects [i]->refscale [i2 * 3 + 1] = 0.25;
                ThreeDObjects [i]->refscale [i2 * 3 + 2] = 0.25;

                if (tmp < 3)
                    {
                    ThreeDObjects [i]->refscale [i2 * 3] = 0;
                    }

                if (tmp < 2)
                    {
                    ThreeDObjects [i]->refscale [i2 * 3 + 1] = 0;
                    }
                }
        }
    ...
    for (i = 0; i < maxfighter; i ++)
        ...
        else if (ThreeDObjects [i]->id == STATIC_HALL1)
            {
...
void game_quit ()
    ...
    delete sound;
    printf ("\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf ("LINUX AIR COMBAT HAS COMPLETED NORMALLY.\n\n");
    printf ("A complete log of all diagnostic messages from this session is available\n");
    printf ("in a file named logfile.txt which you can find in the hidden .LAC folder\n");
    printf ("beneath your home folder. This can be very handy as you configure for best\n");
    printf ("performance with your joystick, keyboard, mouse, network, and display.\n\n");
    printf ("Online help is available at AskMisterWizard.com/FlightSimMovies/LAC\n\n");

    if (strcmp(CommunityHandle, "00000000000000") == 0 )
       {
       printf ("Your CommunityHandle has not yet been set, so other LAC players will never\n");
       printf ("know your name. You should choose a name for yourself and then use a simple\n");
       printf ("text editor to write it into the LacConfig.txt file that you will find in\n");
       printf ("the hidden .LAC folder within your home directory. If you search for\n");
       printf ("CommunityHandle within that file you will find detailed instructions.\n\n");
       }
    exit (EXIT_NORMAL);
    }
...
char *getModelName (int id)
    ...
    if (id == FIGHTER_P38L)
        {
        return "USA LOCKHEED P38L LIGHTNING";
        }
    else if (id == FIGHTER_P51D)
        {
        return "USA NORTH AMERICAN P51D MUSTANG";
        }
    else if (id == FIGHTER_A6M2)
        {
        return "JAPAN MITSUBISHI A6M2 ZERO";
        }
    else if (id == FIGHTER_HAWK)
        {
        return "WHIMSICAL HAWK";
        }
    else if (id == FIGHTER_SPIT9)
        {
        return "UK SUPERMARINE SPITFIRE MK9";
        }
    else if (id == FIGHTER_HURRICANE)
        {
        return "UK HAWKER HURRICANE";
        }
    else if (id == FIGHTER_KI43)
        {
        return "JAPAN NAKAJIMA KI43 OSCAR";
        }
    else if (id == BOMBER_B17)
        {
        return "USA BOEING B17G FLYING FORTRESS";
        }
    else if (id == FIGHTER_F4F)
        {
        return "USA GRUMMAN F4F WILDCAT";
        }
    else if (id == FIGHTER_F6F)
        {
        return "USA GRUMMAN F6F HELLCAT";
        }
    else if (id == FIGHTER_ME109G)
        {
        return "GERMANY MESSERSCHMITT ME109G";
        }
    else if (id == FIGHTER_P39)
        {
        return "USA BELL P39 AIRACOBRA";
        }
    else if (id == FIGHTER_P40)
        {
        return "USA CURTISS P40 WARHAWK";
        }
    else if (id == FIGHTER_F4U)
        {
        return "USA VOUGHT F4U CORSAIR";
        }
    else if (id == FIGHTER_P47D)
        {
        return "USA REPUBLIC P47D THUNDERBOLT";
        }
    else if (id == FIGHTER_FW190)
        {
        return "GERMANY FOCKE WULF FW-190 BUTCHERBIRD";
        }
    else if (id == BOMBER_JU87)
        {
        return "GERMANY JUNKERS JU-87 STUKA";
        }
    else if (id == BOMBER_G5M)
        {
        return "JAPAN MITSUBISHI G4M BETTY";
        }
    else if (id == BOMBER_B25)
        {
        return "USA NORTH AMERICAN B25 MITCHELL";
        }
    else if (id == BOMBER_B26)
        {
        return "USA MARTIN B26 MARAUDER";
        }
    else if (id == FIGHTER_IL16)
        {
        return "USSR POLYKARPOV I-16";
        }
    else if (id == FIGHTER_FIATG55)
        {
        return "ITALY FIAT G55 CENTAURO";
        }
    else if (id == FIGHTER_YAK9)
        {
        return "USSR YAKOVLEV YAK9";
        }
    else if (id == FIGHTER_N1K1)
        {
        return "JAPAN KAWANISHI N1K1 SHIDEN";
        }
    else if (id == BOMBER_B24)
        {
        return "USA CONSOLODATED B24 LIBERATOR";
        }
    else if (id == FIGHTER_LA5)
        {
        return "USSR LAVOCHKIN LA5";
        }
    else if (id == FIGHTER_LA7)
        {
        return "USSR LAVOCHKIN LA7";
        }
    else if (id == FIGHTER_IL2)
        {
        return "USSR ILYUSHIN IL2 STURMOVIK";
        }
    else if (id == FIGHTER_MACCIC202)
        {
        return "ITALY MACCI C202 FOLGORE";
        }
    else if (id == BOMBER_LANCASTER)
        {
        return "UK AVRO LANCASTER";
        }
    else if (id == BOMBER_MOSQUITOB)
        {
        return "UK DE HAVILAND MOSQUITO B";
        }
    else if (id == FIGHTER_TYPHOON)
        {
        return "UK HAWKER TYPHOON";
        }
    else if (id == FIGHTER_YAK1)
        {
        return "USSR YAKOVLEV YAK1";
        }
    else if (id == BOMBER_B29)
        {
        return "USA BOEING B29";
        }
    else if (id == FIGHTER_DW520)
        {
        return "FRENCH DEWOITINE 520";
        }
    else if (id == BOMBER_SB2C)
        {
        return "USA CURTISS SB2C HELLDIVER";
        }
    else if (id == BOMBER_TBF)
        {
        return "USA GRUMMAN TBF AVENGER";
        }
    else if (id == FIGHTER_ME163)
        {
        return "GERMANY ME163 KOMET";
        }
    else if (id == FIGHTER_TEMPEST)
        {
        return "UK HAWKER TEMPEST";
        }
    else if (id == FIGHTER_D3A)
        {
        return "JAPAN AICHI D3A VAL";
        }
    else if (id == BOMBER_B5N)
        {
        return "JAPAN NAKAJIMA B5N KATE";
        }
    else if (id == BOMBER_DAUNTLESS)
        {
        return "USA DOUGLAS DAUNTLESS";
        }
    else if (id == FIGHTER_ME110)
        {
        return "GERMANY MESSERSCHMIDT ME110";
        }
    else if (id == BOMBER_DORNIER)
        {
        return "GERMANY DORNIER 17";
        }
    else if (id == BOMBER_HE111)
        {
        return "GERMANY HEINKEL HE111";
        }
    else if (id == BOMBER_JU88)
        {
        return "GERMANY JUNKERS JU88";
        }
    else if (id == FIGHTER_KI84)
        {
        return "JAPAN NAKAJIMA KI84";
        }
    else if (id == FIGHTER_KI61)
        {
        return "JAPAN KAWASAKI KI61";
        }
    else if (id == FIGHTER_GENERIC01)
        {
        return "UNDEFINED GENERIC01";
        }
    else if (id == FIGHTER_A6M5)
        {
        return "JAPAN MITSUBISHI A6M5 ZERO";
        }
    else if (id == FIGHTER_SPIT5)
        {
        return "UK SUPERMARINE SPITFIRE MK5";
        }
    else if (id == FIGHTER_P51B)
        {
        return "USA NORTH AMERICAN P51B MUSTANG";
        }
    else if (id == FIGHTER_P47B)
        {
        return "USA REPUBLIC P47B THUNDERBOLT";
        }
    else if (id == FIGHTER_ME109F)
        {
        return "GERMANY MESSERSCHMIDT ME109F";
        }
    else if (id == FIGHTER_P38F)
        {
        return "USA LOCKHEED P38F";
        }
    else if (id == BOMB01)
        {
        return "BOMBS";
...
int getTrainingIdFromValue (int n)
    ...
    if (n == z ++)
        ...
        {
        return MISSION_TUTORIAL4;
        }
    else if (n == z ++)
        {
        return MISSION_TURKEYSHOOT;
        }
    else if (n == z ++)
        {
        return MISSION_EVERYMANFORHIMSELF;
        }
    else if (n == z ++)
        {
        return MISSION_TEAMSMISSILESANDGUNS;
        }
    else if (n == z ++)
        {
        ...
        {
        return MISSION_TEAMBASE1;
        }
    else if (n == z ++)
        {
        return MISSION_WAVES1;
        }
    else if (n == z ++)
        {
        return MISSION_JETBAIT1;
        }
    else if (n == z ++)
        {
...
void init_display ()
    ...
    float ThetaRotation = -20 + LacTimer/28;
    if (ThetaRotation > 90)
       {
       ThetaRotation = 90;
       }
    rot.a = 295;
    rot.b = 50 - ThetaRotation;
    rot.c = 00;
    float ZDepth = -80 +LacTimer/40;
    if (ZDepth > -2.5)
       {
       ZDepth = -2.5;
       }
    glTranslatef (0, 0.2, ZDepth);
    ...
    glDisable (GL_BLEND);
    font1->zoom = 0.18;
    font1->drawTextCentered(0, 11, -3, VERSIONSTRING, &red);
    font1->zoom = 0.08;
    font1->drawTextCentered(0, 22,-3,"BY ROBERT BOSEN.");
    font1->drawTextCentered(0,-20,-3,"DERIVED FROM THOMAS A. DREXL'S GL-117");
    font1->drawTextCentered(0,-28,-4,"NO MICROSOFT PRODUCTS WERE USED IN THE DEVELOPMENT OF THIS SOFTWARE.");
    font1->drawTextCentered(0,-30,-4,"NO APPLE PRODUCTS WERE USED IN THE DEVELOPMENT OF THIS SOFTWARE.");
    font1->zoom = 0.1;
    }
...
void KeyGame (int key, int x, int y)
    {
    void switch_menu ();
    void ReshapeGame ();
    ...
    if (camera == 50 && game == GAME_PAUSE)
        {
        display ("DebugRjb170825aa Camera Reset.", LOG_MOST);
        camera = 0;
        game = GAME_PLAY;
        return;
        }
    if (key == 557)
       ...
       if (!CapsLockActive)
          {
          CapsLockActive = true;
          display ("KeyGame() setting CapsLockActive true.", LOG_MOST);
          }
       }
    if (key == 27)
        {
...
void KeyupGame (int key, int x, int y)
    {
    ...
    if (key == 557)
      ...
      if (CapsLockActive)
         ...
         display ("KeyupGame() setting CapsLockActive false.", LOG_MOST);
         }
      }
    if (key > 512)
       {
    ...
    if (key == key_LookBK || key == key_LookDN || key == key_LookLT || key == key_LookRT || key == key_LookUP )
       {
       event_LookFD();
       display ("DebugRjb170825ab Camera Reset.", LOG_MOST);
       camera = 0;
       }
...
static void LacDisplayFunc ()
    ...
    if (game == GAME_INIT)
        ...
        if (camera == 50)
            {
            display ("DebugRjb170825ac Camera Reset.", LOG_MOST);
            camera = 0;
            }
        ...
        if (camera == 50)
            {
            display ("DebugRjb170825ad Camera Reset.", LOG_MOST);
            camera = 0;
            }
        ...
        if (camera == 50)
            {
            display ("DebugRjb170825ae Camera Reset.", LOG_MOST);
            camera = 0;
            }
        ...
        if (camera == 50)
            {
            display ("DebugRjb170825af Camera Reset.", LOG_MOST);
            camera = 0;
            }
...
void LacFirstInit ()
    ...
    textree = gl->genTextureTGA (dirs->getTextures ("tree2.tga"), 0, -1, 1, true);
    textreeu = gl->genTextureTGA (dirs->getTextures ("treeu2.tga"), 0, -1, 1, true);
    textree2 = gl->genTextureTGA (dirs->getTextures ("tree1.tga"), 0, -1, 1, true);
    textreeu2 = gl->genTextureTGA (dirs->getTextures ("treeu1.tga"), 0, -1, 1, true);
    ...
    g_Load3ds.Import3DS (&model_fig, dirs->getModels ("p38Laa.3ds"));
    ...
    g_Load3ds.Import3DS (&model_figi, dirs->getModels ("spitfire.3ds"));
    model_figi.setName ("SPITFIRE");
    display ((char *)" * b17aa.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figu, dirs->getModels ("b17aa.3ds"));
    model_figu.setName ("B17");
    display ((char *)" * a6m2.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figv, dirs->getModels ("a6m2.3ds"));
    model_figv.setName ("A6M2");
    display ((char *)" * P51.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figc, dirs->getModels ("P51a00.3ds"));
    model_figc.setName ("P51D");
    display ((char *)" * P47.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figw, dirs->getModels ("P47aa.3ds"));
    model_figw.setName ("P47D");
    display ((char *)" * F4F.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAO, dirs->getModels ("F4F.3ds"));
    model_figx.setName ("F4F");
    display ((char *)" * F6F.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figx, dirs->getModels ("F6F.3ds"));
    model_figx.setName ("F6F");
    display ((char *)" * F4U.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figy, dirs->getModels ("F4U.3ds"));
    model_figy.setName ("F4U");
    display ((char *)" * FW190.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figz, dirs->getModels ("FW190.3ds"));
    model_figz.setName ("FW190");
    display ((char *)" * JU87.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAA, dirs->getModels ("JU87.3ds"));
    model_figAA.setName ("jU87");
    display ((char *)" * IL16.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAB, dirs->getModels ("IL16.3ds"));
    model_figAB.setName ("IL16");
    display ((char *)" * FIATG55.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAC, dirs->getModels ("FIATG55.3ds"));
    model_figAC.setName ("FIATG55");
    display ((char *)" * ME109.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAD, dirs->getModels ("ME109.3ds"));
    model_figAD.setName ("ME109");
    display ((char *)" * P40.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAE, dirs->getModels ("P40.3ds"));
    model_figAE.setName ("P40");
    display ((char *)" * HURRICANE.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAF, dirs->getModels ("HURRICANE.3ds"));
    model_figAF.setName ("HURRICANE");
    display ((char *)" * KI43.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAG, dirs->getModels ("KI43.3ds"));
    model_figAG.setName ("KI43");
    display ((char *)" * Yak9.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAH, dirs->getModels ("Yak9.3ds"));
    model_figAH.setName ("YAK9");
    display ((char *)" * N1K1.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAI, dirs->getModels ("N1K1.3ds"));
    model_figAI.setName ("N1K1");
    display ((char *)" * b24.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAJ, dirs->getModels ("b24.3ds"));
    model_figu.setName ("B24");
    display ((char *)" * P39.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAK, dirs->getModels ("P39.3ds"));
    model_figAK.setName ("P38");
    display ((char *)" * G4m.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAL, dirs->getModels ("G4m.3ds"));
    model_figAL.setName ("G4M");
    display ((char *)" * B25.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAM, dirs->getModels ("B25.3ds"));
    model_figAM.setName ("B25");
    display ((char *)" * B26.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAN, dirs->getModels ("B26.3ds"));
    model_figAN.setName ("B26");
    display ((char *)" * La5.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAP, dirs->getModels ("La5.3ds"));
    model_figAP.setName ("LA5");
    display ((char *)" * La7.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAQ, dirs->getModels ("La7.3ds"));
    model_figAQ.setName ("LA7");
    display ((char *)" * IL2.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAR, dirs->getModels ("IL2.3ds"));
    model_figAR.setName ("IL2");
    display ((char *)" * MacciC202.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAS, dirs->getModels ("MacciC202.3ds"));
    model_figAS.setName ("MACCI");
    display ((char *)" * Lancaster.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAT, dirs->getModels ("Lancaster.3ds"));
    model_figAT.setName ("LANCASTER");
    display ((char *)" * MosquitoB.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAU, dirs->getModels ("MosquitoB.3ds"));
    model_figAU.setName ("MOSQUIOB");
    display ((char *)" * Typhoon.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAV, dirs->getModels ("Typhoon.3ds"));
    model_figAV.setName ("Typhoon");
    display ((char *)" * Yak1.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAW, dirs->getModels ("Yak1.3ds"));
    model_figAW.setName ("YAK1");
    display ((char *)" * B29.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAX, dirs->getModels ("B29.3ds"));
    model_figAX.setName ("B29");
    display ((char *)" * DW520.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAY, dirs->getModels ("DW520.3ds"));
    model_figAY.setName ("DW520");
    display ((char *)" * SB2C.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figAZ, dirs->getModels ("SB2C.3ds"));
    model_figAZ.setName ("SB2C");
    display ((char *)" * TBF.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBA, dirs->getModels ("TBF.3ds"));
    model_figBA.setName ("TBF");
    display ((char *)" * ME163.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBB, dirs->getModels ("ME163.3ds"));
    model_figBB.setName ("ME163");
    display ((char *)" * TEMPEST.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBC, dirs->getModels ("TEMPEST.3ds"));
    model_figBC.setName ("TEMPEST");
    display ((char *)" * D3A.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBD, dirs->getModels ("D3A.3ds"));
    model_figBD.setName ("D3A");
    display ((char *)" * B5N.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBE, dirs->getModels ("B5N.3ds"));
    model_figBD.setName ("B5N");
    display ((char *)" * Dauntless.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBF, dirs->getModels ("Dauntless.3ds"));
    model_figBF.setName ("Dauntless");
    display ((char *)" * Me110.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBG, dirs->getModels ("ME110.3ds"));
    model_figBG.setName ("ME110");
    display ((char *)" * DORNIER.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBH, dirs->getModels ("Dornier17.3ds"));
    model_figBH.setName ("DORNIER");
    display ((char *)" * He111.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBI, dirs->getModels ("HE111.3ds"));
    model_figBI.setName ("HE111");
    display ((char *)" * JU88.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBJ, dirs->getModels ("JU88.3ds"));
    model_figBJ.setName ("JU88");
    display ((char *)" * KI84.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBK, dirs->getModels ("KI84.3ds"));
    model_figBK.setName ("KI84");
    display ((char *)" * KI61.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBL, dirs->getModels ("KI61.3ds"));
    model_figBL.setName ("KI61");
    display ((char *)" * Generic01.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBM, dirs->getModels ("Generic01.3ds"));
    model_figAS.setName ("GENERIC01");
    display ((char *)" * A6M5.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBN, dirs->getModels ("a6m5.3ds"));
    model_figAT.setName ("A6M5");
    display ((char *)" * SPIT5.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBO, dirs->getModels ("SPIT5.3ds"));
    model_figAU.setName ("SPIT5");
    display ((char *)" * P51B.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBP, dirs->getModels ("P51B.3ds"));
    model_figAV.setName ("P51B");
    display ((char *)" * P47B.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBQ, dirs->getModels ("P47B.3ds"));
    model_figAW.setName ("P47B");
    display ((char *)" * ME109F.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBR, dirs->getModels ("ME109F.3ds"));
    model_figAX.setName ("ME109F");
    display ((char *)" * P38F.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_figBS, dirs->getModels ("P38F.3ds"));
    model_figAY.setName ("P38F");
    ...
    g_Load3ds.Import3DS (&model_bomb01, dirs->getModels ("bomb1.3ds"));
    ...
    display ((char *)" * ship2.3ds", LOG_MOST);
    ...
    g_Load3ds.Import3DS (&model_RadarReflector, dirs->getModels ("RadarReflector01.3ds"));
    model_RadarReflector.setName ("HQRADAR");
    display ((char *)" * Airfield00.3ds", LOG_MOST);
    g_Load3ds.Import3DS (&model_Airfield00, dirs->getModels ("Airfield00.3ds"));
    ...
    textitle = gl->genTextureTGA (dirs->getTextures ("LacTitle.tga"), 0, 0, 0, false);
    //gl->genTextureTGA()
    sungamma = 60;
    setLightSource (10);
    event_setAntialiasing ();
...
static void LacKeyboardFunc (unsigned char uckey, int x, int y)
    ...
    if (game == GAME_PLAY || game == GAME_PAUSE)
        ...
        if (EnterPressed)
           {
           display ("LacKeyboardFunc() EnterPressed = true", LOG_MOST);;
           }
        else
           {
           display ("LacKeyboardFunc() EnterPressed = false", LOG_MOST);;
           }
        if (SpaceBarPressed)
           {
           display ("LacKeyboardFunc() SpaceBarPressed = true", LOG_MOST);;
           }
        else
           {
           display ("LacKeyboardFunc() SpaceBarPressed = false", LOG_MOST);;
           }
        if (SpaceBarPressed || EnterPressed)
           {
           MouseMission(1, 1, mousex, mousey);
           display ("LacKeyboardFunc() invoking MouseMission(1, 1)", LOG_MOST);
           }
        else
           {
           MouseMission(1, 0, mousex, mousey);
           display ("LacKeyboardFunc() invoking MouseMission(1, 0)", LOG_MOST);
           }
        }
    else if (game == GAME_STATS)
        {
...
static void LacKeyboardFuncUp (unsigned char key, int x, int y)
    ...
    if (key == 32)
       ...
       display ("Setting SpaceBarPressed true.", LOG_MOST);
       }
    else
       {
       SpaceBarPressed = false;
       display ("Setting SpaceBarPressed false.", LOG_MOST);
       }
    if (key == 13)
       {
       EnterPressed = true;
       display ("Setting EnterPressed true.", LOG_MOST);
       }
    else
       {
       EnterPressed = false;
       display ("Setting EnterPressed false.", LOG_MOST);
       }
    }

static void LacMouseFunc (int button, int state, int x, int y)
    {
    display ("Function entry: LacMouseFunc()", LOG_MOST);
    // Abstract Event Branch
    void MouseMission (int button, int state, int x, int y);
...
void LacReshapeFunc (int width, int height)
    {
    display ("Function entry: LacReshapFunc()", LOG_MOST);
    // Abstract Event Branch
    void ReshapeGame ();
    void ReshapeMenu ();
    void ReshapeCredits ();
...
void mission_display ()
    ...
    if (missionnew == NULL)
       {
       display ("mission_display() alert. missionnew is null.", LOG_MOST);
       }
    tl.x = -0.16;
    ...
    float xstatstab = 5;
    font1->drawText (xstats / fontscale, ystats / fontscale, -2, "MISSION NOTES:", col);

    if (CurrentMissionNumber == MISSION_NETWORKBATTLE01 || CurrentMissionNumber == MISSION_NETWORKBATTLE02 || CurrentMissionNumber == MISSION_NETWORKBATTLE03)
        {
        font1->drawText (xstats / fontscale, (ystats -1) / fontscale, -2, "1 HEAVY BOMBER CAN DESTROY\nAN AIRBASE IN 2 SORTIES,\nBUT WITHOUT A BOMBER,\nMANY FIGHTERS ARE NEEDED.\n\nIF YOU ARE BEING VULCHED,\nUSE A DIFFERENT AIRFIELD.", col);
        }
    else
        {
        font1->drawText (xstats / fontscale, (ystats -1) / fontscale, -2, "NO FURTHER DETAILS", col);
        }

    font1->drawText (textx / fontscale, -1 / fontscale, -2, "CURRENT AIRCRAFT:", col);
    font2->drawText (textx / fontscale, -6 / fontscale, -2, getModelName (missionnew->selfighter [missionnew->wantfighter]), col);

    if (missionnew->selweapons)
        {
        font1->drawText (xstats / fontscale, -1 / fontscale, -2, "SELECT WEAPONS:", col);
        font2->drawText (xstats / fontscale, -6 / fontscale, -2, getModelName (missionnew->selweapon [missionnew->wantweapon]), col);
        }
    else
        {
        font1->drawText (xstats / fontscale, -1 / fontscale, -2, "SELECT STARTING AIRFIELD\n(CLICK AIRFIELD NAME BELOW):", col);
        if (AirfieldRequested == 1)
           {
           if (MyNetworkId%2)
              {
              font1->drawText (xstats / fontscale, -4 / fontscale, -2, "NEAR THE ENEMY (RADAR BLIP 29): \nRED HQ AIRFIELD \n(BOMBERS 50% LOAD)", col);
              }
           else
              { // Player is on BlueTeam
              font1->drawText (xstats / fontscale, -4 / fontscale, -2, "NEAR THE ENEMY (RADAR BLIP 28): \nBLUE HQ AIRFIELD \n(BOMBERS 50% LOAD)", col);
              }
           }
           ...
           if (MyNetworkId%2)
              {
              font1->drawText (xstats / fontscale, -4 / fontscale, -2, "FARTHER OUT: \nRED AIRFIELD 2 (ADJACENT)\n(BOMBERS 75% LOAD)", col);
              }
           else
              {
...
void MouseMission (int button, int state, int x, int y)
    ...
    if (ry >= 0.53 && ry <= 0.65)
        ...
        if (rx >= 0.18 && rx < 0.28)
            ...
            if (state == MOUSE_DOWN)
                {
                display ("MouseMission() area 1 click detected", LOG_MOST);
        ...
        if (rx >= 0.35 && rx < 0.48)
            ...
            if (state == MOUSE_DOWN)
                {
                display ("MouseMission() area 2 click detected", LOG_MOST);
    ...
    if (ry >= 0.53 && ry <= 0.65)
        ...
        if (rx >= 0.54 && rx < 0.64)
            ...
            if (state == MOUSE_DOWN)
                {
                display ("MouseMission() area 3 click detected", LOG_MOST);
                if (CurrentMissionNumber == MISSION_NETWORKBATTLE01 || CurrentMissionNumber == MISSION_NETWORKBATTLE02 || CurrentMissionNumber == MISSION_NETWORKBATTLE03)
                   {
                   display ("MouseMission() AirfieldSelectCommand detected", LOG_MOST);
                   AirfieldRequested ++;
                   if (AirfieldRequested >=4)
                      {
                      AirfieldRequested = 1;
                      }
                   display ("MouseMission() AirfieldRequested =", LOG_MOST);
        ...
        if (rx >= 0.64 && rx < 0.74 && missionnew->selweapons >= 2)
            ...
            if (state == MOUSE_DOWN)
                {
                display ("MouseMission() area 4 click detected", LOG_MOST);
        ...
        if (rx >= 0.74 && rx < 0.84 && missionnew->selweapons >= 3)
            ...
            if (state == MOUSE_DOWN)
                {
                display ("MouseMission() area 5 click detected", LOG_MOST);
...
void pause_display ()
    ...
    glLoadIdentity ();
    glPushMatrix ();
    font1->drawTextCentered (0, 4, -2, "PAUSED. PRESS ESCAPE.", &colorwhite);
    glPopMatrix ();
    }
...
void playRandomMusic ()
    ...
    if (r == 0)
        {
        display ("playRandomMusic() MUSIC_MUSIC00", LOG_MOST);
        sound->loadMusic (MUSIC_MUSIC01);
        }
    else if (r == 1)
        {
        display ("playRandomMusic() MUSIC_MUSIC01", LOG_MOST);
        sound->loadMusic (MUSIC_MUSIC01);
        }
    else if (r == 2)
        {
        display ("playRandomMusic() MUSIC_MUSIC02", LOG_MOST);
        sound->loadMusic (MUSIC_MUSIC02);
        }
    else if (r == 3)
        {
        display ("playRandomMusic() MUSIC_MUSIC03", LOG_MOST);
...
void sdlMainLoop ()
    ...
    while (true)
        ...
        if (game == GAME_PLAY) {
            ...
            CVector3 vaxis;
            vaxis.set (COS(fplayer->gamma) * SIN(fplayer->phi), -1 * SIN(fplayer->gamma), COS(fplayer->gamma) * COS(fplayer->phi));

            CVector3 idealaxis;
            idealaxis.set((ThreeDObjects[1]->tl->x - fplayer->tl->x) / fplayer->distance(ThreeDObjects[1]), (ThreeDObjects[1]->tl->y - fplayer->tl->y) / fplayer->distance(ThreeDObjects[1]), (ThreeDObjects[1]->tl->z - fplayer->tl->z) / fplayer->distance(ThreeDObjects[1]));
            ...
            */
            ...
            std::cout << "fplayer->tl->x: " << fplayer->tl->x << std::endl;
            std::cout << "fplayer->tl->y: " << fplayer->tl->y << std::endl;
            std::cout << "fplayer->tl->z: " << fplayer->tl->z << std::endl;
            std::cout << "vaxis->x: " << vaxis.x << std::endl;
...
*/
        ...
        if (game == GAME_PLAY)
			...
			//Output player effects:
			std::cout << "Rudder effect:" << std::endl;
			std::cout << fplayer->ruddereffect << std::endl;

			std::cout << "Elevator effect:" << std::endl;
			std::cout << fplayer->elevatoreffect << std::endl;

			std::cout << "Roll effect:" << std::endl;
			std::cout << fplayer->rolleffect << std::endl;
...
void switch_fighter ()
    {
    display ("switch_fighter()", LOG_MOST);
    LastTime = 0;
    setLightSource (60);
    game = GAME_FIGHTER;
    ReshapeMenu ();
...
void switch_menu ()
    {
    display ("Function entry: switch_menu()", LOG_MOST);
    LastTime = 0;
    setLightSource ((int) sungamma);
    game = GAME_MENU;
    ReshapeMenu ();
...
void switch_mission (int missionid)
    {
    display ("switch_mission(). missionid = ", LOG_MOST);
...
void TargetVocalizeProcess1 ()
   ...
   int MissionAircraft;
   display ("TargetVocalizeProcess1()", LOG_ALL);
   ...
   if (TargetVocalize1State == 1  & !State1Vocalized)
      ...
      State1Vocalized = false;
         ...
         if (InterceptAngle < 12)
            {
            sound->play (SOUND_DIGIT012, false);
            }
         else if (InterceptAngle < 30)
            {
            sound->play (SOUND_DIGIT001, false);
            }
         else if (InterceptAngle < 60)
            {
            sound->play (SOUND_DIGIT002, false);
            }
         else if (InterceptAngle < 90)
            {
            sound->play (SOUND_DIGIT003, false);
            }
         else if (InterceptAngle < 120)
            {
            sound->play (SOUND_DIGIT004, false);
            }
         else if (InterceptAngle < 150)
            {
            sound->play (SOUND_DIGIT005, false);
            }
         else if (InterceptAngle < 180)
            {
            sound->play (SOUND_DIGIT006, false);
            }
         else if (InterceptAngle < 210)
            {
            sound->play (SOUND_DIGIT007, false);
            }
         else if (InterceptAngle < 240)
            {
            sound->play (SOUND_DIGIT008, false);
            }
         else if (InterceptAngle < 270)
            {
            sound->play (SOUND_DIGIT009, false);
            }
         else if (InterceptAngle < 300)
            {
            sound->play (SOUND_DIGIT010, false);
            }
         else if (InterceptAngle < 330)
            {
         ...
         {
         if ((fplayer->target->tl->y / 25.0) < 0.5)
            {
            sound->setVolume (SOUND_DIGIT000, 90);
            sound->play (SOUND_DIGIT000, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 1.0)
            {
            sound->setVolume (SOUND_DIGIT001, 90);
            sound->play (SOUND_DIGIT001, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 2)
            {
            sound->setVolume (SOUND_DIGIT002, 90);
            sound->play (SOUND_DIGIT002, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 3)
            {
            sound->setVolume (SOUND_DIGIT003, 90);
            sound->play (SOUND_DIGIT003, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 4)
            {
            sound->setVolume (SOUND_DIGIT004, 90);
            sound->play (SOUND_DIGIT004, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 5)
            {
            sound->setVolume (SOUND_DIGIT005, 90);
            sound->play (SOUND_DIGIT005, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 6)
            {
            sound->setVolume (SOUND_DIGIT006, 90);
            sound->play (SOUND_DIGIT006, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 7)
            {
            sound->setVolume (SOUND_DIGIT007, 90);
            sound->play (SOUND_DIGIT007, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 8)
            {
            sound->setVolume (SOUND_DIGIT008, 90);
            sound->play (SOUND_DIGIT008, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 9)
            {
            sound->setVolume (SOUND_DIGIT009, 90);
            sound->play (SOUND_DIGIT009, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 10)
            {
            sound->setVolume (SOUND_DIGIT010, 90);
            sound->play (SOUND_DIGIT010, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 11)
            {
            sound->setVolume (SOUND_DIGIT011, 90);
            sound->play (SOUND_DIGIT011, false);
            }
         else if ((fplayer->target->tl->y / 25.0) <12)
            {
            sound->setVolume (SOUND_DIGIT012, 90);
            sound->play (SOUND_DIGIT012, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 13)
            {
            sound->setVolume (SOUND_DIGIT013, 90);
            sound->play (SOUND_DIGIT013, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 14)
            {
            sound->setVolume (SOUND_DIGIT014, 90);
            sound->play (SOUND_DIGIT014, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 15)
            {
            sound->setVolume (SOUND_DIGIT015, 90);
            sound->play (SOUND_DIGIT015, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 16)
            {
            sound->setVolume (SOUND_DIGIT016, 90);
            sound->play (SOUND_DIGIT016, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 17)
            {
            sound->setVolume (SOUND_DIGIT017, 90);
            sound->play (SOUND_DIGIT017, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 18)
            {
            sound->setVolume (SOUND_DIGIT018, 90);
            sound->play (SOUND_DIGIT018, false);
            }
         else if ((fplayer->target->tl->y / 25.0) < 19)
            {
            sound->setVolume (SOUND_DIGIT019, 90);
            sound->play (SOUND_DIGIT019, false);
            }
         else if (((fplayer->target->tl->y / 25.0) > 19) && ((fplayer->target->tl->y / 25.0) < 29))
            {
            sound->setVolume (SOUND_DIGIT020, 90);
            sound->play (SOUND_DIGIT020, false);
            }
         else if ((fplayer->target->tl->y / 25.0) > 29)
            {
         ...
         {
         if (((fplayer->target->tl->y / 25.0) > 20) && ((fplayer->target->tl->y / 25.0) < 30))
            {
            if ((fplayer->target->tl->y / 25.0) < 21)
               {
               sound->setVolume (SOUND_DIGIT001, 90);
               sound->play (SOUND_DIGIT001, false);
               }
            ...
            else if ((fplayer->target->tl->y / 25.0) < 23)
               {
               sound->setVolume (SOUND_DIGIT003, 90);
               sound->play (SOUND_DIGIT003, false);
               }
            ...
            else if ((fplayer->target->tl->y / 25.0) < 25)
               {
               sound->setVolume (SOUND_DIGIT005, 90);
               sound->play (SOUND_DIGIT005, false);
               }
            ...
            else if ((fplayer->target->tl->y / 25.0) < 27)
               {
               sound->setVolume (SOUND_DIGIT007, 90);
               sound->play (SOUND_DIGIT007, false);
               }
         ...
         else if (((fplayer->target->tl->y / 25.0) > 30) && ((fplayer->target->tl->y / 25.0) < 40))
            {
            if ((fplayer->target->tl->y / 25.0) < 31)
               {
               sound->setVolume (SOUND_DIGIT001, 90);
               sound->play (SOUND_DIGIT001, false);
               }
            ...
            else if ((fplayer->target->tl->y / 25.0) < 33)
               {
               sound->setVolume (SOUND_DIGIT003, 90);
               sound->play (SOUND_DIGIT003, false);
               }
            ...
            else if ((fplayer->target->tl->y / 25.0) < 35)
               {
               sound->setVolume (SOUND_DIGIT005, 90);
               sound->play (SOUND_DIGIT005, false);
               }
            ...
            else if ((fplayer->target->tl->y / 25.0) < 37)
               {
               sound->setVolume (SOUND_DIGIT007, 90);
               sound->play (SOUND_DIGIT007, false);
               }
            ...
            else if ((fplayer->target->tl->y / 25.0) < 39)
               {
               sound->setVolume (SOUND_DIGIT009, 90);
               sound->play (SOUND_DIGIT009, false);
               }
...
void TimerGame (int dt)
    ...
    for (i = 0; i < maxfighter; i ++)
        ...
        for (i2 = 0; i2 < maxlaser; i2 ++)
            if (laser [i2]->active)
                {
                ThreeDObjects [i]->collide (laser [i2], dt);
                }
    ...
    for (i = 0; i < maxmissile; i ++)
        {
        missile [i]->aiAction (dt, (AIObj **) ThreeDObjects, missile, laser, flare, chaff);
        }
...
void TimerMenu (Uint32 dt)
    ...
    if (cycle == 0)
        {
        display ("DebugRjb170825ag Camera Reset.", LOG_MOST);
        newcamera = 0;
        }
...
{
sprintf (DebugBuf, "TransmitCharacterViaMorseRadio() %c", (char)key);
display (DebugBuf, LOG_MOST);
MorseCharacterOut = (char)key;
}
...
if (!ShotDownByBomberOrAirfieldGunners)
   {
   display ("UpdateOnlineScoreLogFileWithCalculatedRisks()\n", LOG_MOST);
   time_t now = time(NULL);
   struct tm *CurrentTimeStruct = gmtime(&now);
...
if (!LandingLogged)
   {
   LandingLogged = true;
   display ("UpdateOnlineScoreLogFileWithCrashes()\n", LOG_MOST);
   time_t now = time(NULL);
   struct tm *CurrentTimeStruct = gmtime(&now);
...
if (!LandingLogged)
   {
   LandingLogged = true;
   display ("UpdateOnlineScoreLogFileWithNewLandings()\n", LOG_MOST);
   time_t now = time(NULL);
   struct tm *CurrentTimeStruct = gmtime(&now);
...
void UpdateOnlineScoreLogFileWithNewSession()
{
display ("UpdateOnlineScoreLogFileWithNewSession()\n", LOG_MOST);
time_t now = time(NULL);
struct tm *CurrentTimeStruct = gmtime(&now);
...
void UpdateOnlineScoreLogFileWithNewSorties()
{
display ("UpdateOnlineScoreLogFileWithNewSorties()\n", LOG_MOST);
char AircraftName[32];
time_t now = time(NULL);
struct tm *CurrentTimeStruct = gmtime(&now);
...
if (!TakeoffLogged)
   {
   TakeoffLogged = true;
   display ("UpdateOnlineScoreLogFileWithTakeoffs()\n", LOG_MOST);
   time_t now = time(NULL);
   struct tm *CurrentTimeStruct = gmtime(&now);
...
void VocalizeRedHqStatus ()
{
display ("VocalizeRedHqStatus()", LOG_MOST);
 if (ThreeDObjects[29]->Durability <= ThreeDObjects[29]->maxDurability * 0.1)
    {
    display ("VocalizeRedHqStatus() SOUND_ENEMYHQACKACKFROM01TO10PCT", LOG_MOST);
    sound->play (SOUND_ENEMYHQACKACKFROM01TO10PCT, false);
    return;
    }
 else if (ThreeDObjects[29]->Durability <= ThreeDObjects[29]->maxDurability * 0.2)
    {
    display ("VocalizeRedHqStatus() SOUND_ENEMYHQACKACKFROM10TO20PCT", LOG_MOST);
    sound->play (SOUND_ENEMYHQACKACKFROM10TO20PCT, false);
    return;
    }
 else if (ThreeDObjects[29]->Durability <= ThreeDObjects[29]->maxDurability * 0.3)
    {
    display ("VocalizeRedHqStatus() SOUND_ENEMYHQACKACKFROM20TO30PCT", LOG_MOST);
    sound->play (SOUND_ENEMYHQACKACKFROM20TO30PCT, false);
    return;
    }
 else if (ThreeDObjects[29]->Durability <= ThreeDObjects[29]->maxDurability * 0.4)
    {
    display ("VocalizeRedHqStatus() SOUND_ENEMYHQACKACKFROM30TO40PCT", LOG_MOST);
    sound->play (SOUND_ENEMYHQACKACKFROM30TO40PCT, false);
    return;
    }
 else if (ThreeDObjects[29]->Durability <= ThreeDObjects[29]->maxDurability * 0.5)
    {
    display ("VocalizeRedHqStatus() SOUND_ENEMYHQACKACKFROM40TO50PCT", LOG_MOST);
    sound->play (SOUND_ENEMYHQACKACKFROM40TO50PCT, false);
    return;
    }
 else if (ThreeDObjects[29]->Durability <= ThreeDObjects[29]->maxDurability * 0.6)
    {
    display ("VocalizeRedHqStatus() SOUND_ENEMYHQACKACKFROM50TO60PCT", LOG_MOST);
    sound->play (SOUND_ENEMYHQACKACKFROM50TO60PCT, false);
    return;
    }
 else if (ThreeDObjects[29]->Durability <= ThreeDObjects[29]->maxDurability * 0.7)
    {
    display ("VocalizeRedHqStatus() SOUND_ENEMYHQACKACKFROM60TO70PCT", LOG_MOST);
    sound->play (SOUND_ENEMYHQACKACKFROM60TO70PCT, false);
    return;
    }
 else if (ThreeDObjects[29]->Durability <= ThreeDObjects[29]->maxDurability * 0.8)
    {
    display ("VocalizeRedHqStatus() SOUND_ENEMYHQACKACKFROM70TO80PCT", LOG_MOST);
    sound->play (SOUND_ENEMYHQACKACKFROM70TO80PCT, false);
    return;
    }
 else if (ThreeDObjects[29]->Durability <= ThreeDObjects[29]->maxDurability * 0.9)
    {
    display ("VocalizeRedHqStatus() SOUND_ENEMYHQACKACKFROM80TO90PCT", LOG_MOST);
    sound->play (SOUND_ENEMYHQACKACKFROM80TO90PCT, false);
    return;
    }
 else if (ThreeDObjects[29]->Durability <= ThreeDObjects[29]->maxDurability)
    {
    display ("VocalizeRedHqStatus() SOUND_ENEMYHQACKACKFROM90TO100PCT", LOG_MOST);
    sound->play (SOUND_ENEMYHQACKACKFROM90TO100PCT, false);
    }
}
