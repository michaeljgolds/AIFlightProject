...
extern unsigned char MorseCharacterOut;
...
extern Uint16 NetworkPasswordHash;
...
LacUdpApiPacket PriorInPacket[32];    
 ...
 void AdjustHeightAboveRunwayIfLanded()
    ...
    float TerrainHeightHere = l->getExactHeight (fplayer->tl->x, fplayer->tl->z);
    float PlayerAlt = fabs(fplayer->tl->y);
    float HeightAboveRunway = PlayerAlt - TerrainHeightHere;
...
void LoadServerIpAddress()
{
   ...
   { 
   display ("LoadServerIpAddress() did not find a configured DNS name in LacConfig.txt", LOG_NET);
   ...
   {
   display ("LoadServerAddress() found text in LacConfig.txt that might be a valid DNS name.", LOG_NET);
...
display ("ServerIpAddressAscii=", LOG_NET);
display (ServerIpAddressAscii, LOG_NET);
display ("Looking up Hostname:", LOG_NET);
hostlist = gethostbyname (ServerIpAddressAscii);
if (hostlist == NULL)
   {
   display ("LoadServerIpAddress() unable to resolve host name", LOG_NET);
   }
else
   { 
   display ("LoadServerIpAddress() resolved to valid hostlist.", LOG_NET);
   if (hostlist->h_addrtype != AF_INET)
      { 
      display ("Server IP Address is not properly formatted for IPV4.", LOG_NET);
      }
   
   inet_ntop (AF_INET, hostlist->h_addr_list[0], ServerIpAddressDottedDecimal, 15);
   display ("LoadServerIpAddress() Resolved Server IP to:", LOG_NET);
...
if (ClientSocketHandle < 0)
   {
   display ("OpenClientUdpSocket() unable to open requested Datagram Socket", LOG_NET);
   return (1);
   }
   else
   {
...
if (OutSocketHandle < 0)
   {
   display ("NetworkApi.cpp OpenUdpSocketForSending() unable to open requested Datagram Socket", LOG_NET);
   return (1);
   }
   else
   {
...
if (InSocketHandle < 0)
   {
    display ("NetworkApi.cpp OpenUdpSocketForReceiving() unable to open requested Datagram Socket. errno=", LOG_NET);
   ...
   {
   display ("OpenUdpSocketForReceiving() returned 0 indicating success.", LOG_NET);
   display ("InSocketHandle=",LOG_NET);
...
if (BindStatus <0)
   { 
   display ("ConfigureClientUdpSocket() cannot bind to an ephemeral UDP port.", LOG_NET);
   sprintf (DebugBuf, "%s", strerror(errno));
   display (DebugBuf, LOG_NET);
   }
   else
   { 
   display ("ConfigureClientUdpSocket() ClientServer mode bind() succeeded for ClientSourceAddress. BindStatus=", LOG_NET);
   ...
   if((getsockname(ClientSocketHandle, (struct sockaddr *)&ClientSourceAddress, &ClientSourceAddressLength))!=-1)
      ...
      display ("EphemeralClientPort = ", LOG_NET);
      sprintf (DebugBuf, "%d", EphemeralClientPort); 
      display (DebugBuf, LOG_NET);
      }
   else
      { 
      display ("ConfigureClientUdpSocket() ClientServer mode getsockname() failed for ClientSourceAddress.", LOG_NET);
      }
   }
if(NetworkMode == 1)
   { 
   ...
   {
   display ("ConfigureClientUdpSocket() Invalid NetworkMode configured in LacConfig.txt.", LOG_NET);
   }
} 
...
if (NetworkMode == 0)
   ...
   { 
   display ("ConfigureIncomingUdpSocket() InSocketAddress.sin_port=", LOG_NET);
   sprintf (DebugBuf, "%hu", ntohs(InSocketAddress.sin_port));
   display (DebugBuf, LOG_NET);
   }
else
   {
   display ("ConfigUreIncomingUdpSocket() ClientServer mode. Invalid NetworkMode from LacConfig.txt", LOG_NET);
   }
if (NetworkMode == 0)
   { 
   ...
   if (BindStatus <0)
      { 
       display ("ConfigureIncomingUdpSocket() cannot bind to requested UDP port. Port=", LOG_NET);
      ...
      { 
      display ("ConfigureIncomingUdpSocket() PeerToPeer mode bind() succeeded. BindStatus=", LOG_NET);
      sprintf (DebugBuf, "%i", BindStatus);
      display (DebugBuf, LOG_NET);
      display ("InSocketAddress.sin_port = ", LOG_NET);
...
if (NetworkMode == 1)
   ...
   if (BindStatus <0)
      ...
      { 
      display ("ConfigureIncomingUdpSocket() ClientServer mode bind() succeeded. BindStatus=", LOG_NET);
      sprintf (DebugBuf, "%i", BindStatus);
      display (DebugBuf, LOG_NET);
      ...
      if((getsockname(InSocketHandle, (struct sockaddr *)&InSocketAddress, &InSocketAddressLength))!=-1)
         ...
         display ("EphemeralInPort = ", LOG_NET);
         sprintf (DebugBuf, "%d", EphemeralInPort); 
         display (DebugBuf, LOG_NET);
         }
      else
         { 
         display ("ConfigureIncomingUdpSocket() ClientServer mode getsockname() failed.", LOG_NET);
         }
      }
   } 
} 
...
if(NetworkMode==0 || NetworkMode == 1)
   { 
   display ("ConfigureOutgoingUdpSocket() Using LacConfig.txt PeerInPortNumber for Destination port.", LOG_NET);
   OutSocketAddress.sin_port = htons(PeerInPortNumber); 
   DestinationPort = ntohs(OutSocketAddress.sin_port);
   display ("DestinationPort = ", LOG_NET);
   sprintf (DebugBuf, "%d", DestinationPort); 
   display (DebugBuf, LOG_NET);
   }
else
   {
   display ("ConfigureOutgoingUdpSocket() Invalid NetworkMode configured in LacConfig.txt.", LOG_NET);
   }
...
int DiscardAnyInPacketsInQueue()
{
   ...
   while (PacketIsAvailable)
      ...
      if (RecFromStatus == -1)
         ...
         display ("No packet received because:", LOG_NET);
         sprintf (DebugBuf, "%s", strerror(errno));
         display (DebugBuf, LOG_NET);
         return -1; 
         }
...
if (NetworkMode == 0)
   ...
   {
   display ("GetNetworkApiPacket() detected improper value for NetworkMode.", LOG_NET);
   }
if (RecFromStatus == -1)
   { 
   display ("No packet received because:", LOG_NET);
   ...
   if (NetworkMode == 0)
      ...
      if((getsockname(InSocketHandle, (struct sockaddr *)&InSocketAddress, &InSocketAddressLength))!=-1)
         ...
         display ("InSocket EphemeralPort = ", LOG_NET);
         sprintf (DebugBuf, "%d", EphemeralPort);
         display (DebugBuf, LOG_NET);
         }
      else
         { 
         display ("GetNetworkApiPacket() getsockname() failed.", LOG_NET);
         }
      }
   else if (NetworkMode == 1)
      {
      if((getsockname(ClientSocketHandle, (struct sockaddr *)&ClientSourceAddress, &ClientSourceAddressLength))!=-1)
         ...
         display ("ClientSocket EphemeralPort = ", LOG_NET);
         sprintf (DebugBuf, "%d", EphemeralPort);
         display (DebugBuf, LOG_NET);
         }
      else
...
display (DebugBuf, LOG_NET);
if (RecFromStatus <= sizeof(InPacket))
   { 
...
if (InPacket.UdpObjCommand == 0)
   ...
   if (AuthenticateInPacket() == 0)
      ...
      if (InPacket.UdpObjPlayerNumber == MyNetworkId)
        { 
        display ("GetNetworkApiPacket() detected a conflict with the value of MyNetworkId.", LOG_MOST);
        if (MissionOutPacketCount < 252)
           { 
           display ("GetNetworkApiPacket() I think I am the newcomer in this situation.", LOG_MOST);
           MissionIdNegotiationCount++;
           NetworkTransmitTimer -= 1000; 
           display ("GetNetworkApiPacket() NetworkTransmitTimer waiting an additional 1000 ms.", LOG_MOST);
           if (MissionIdNegotiationCount>32)
              { 
              MissionIdNegotiationCount = 33; 
              display ("GetNetworkApiPacket() failed to find an unused value for MyNetworkID for your team.", LOG_MOST);
              display ("This probably means that the mission is full. Try another mission or edit your", LOG_MOST);
              display ("LacConfig.txt file to change your MyNetworkId value to the other team.", LOG_MOST);
              ...
              display ("GetNetworkApiPacket() Reset MissionOutPacketCount.", LOG_MOST);
              display ("GetNetworkApiPacket() is trying to find an available value for MyNetworkID. Now trying", LOG_MOST);
     ...
     if (InPacket.UdpObjTerrainOrMorse > LANDSCAPE_ALPINE_SEA)
      ...
      InPacketDeltaXPosition[InPacket.UdpObjPlayerNumber] = InPacket.UdpObjXPosition - PriorInPacket[InPacket.UdpObjPlayerNumber].UdpObjXPosition;
      InPacketDeltaYPosition[InPacket.UdpObjPlayerNumber] = InPacket.UdpObjYPosition - PriorInPacket[InPacket.UdpObjPlayerNumber].UdpObjYPosition;
      InPacketDeltaZPosition[InPacket.UdpObjPlayerNumber] = InPacket.UdpObjZPosition - PriorInPacket[InPacket.UdpObjPlayerNumber].UdpObjZPosition;
      ...
      if (InPacket.UdpObjDamageAmount > 1000.0 && InPacket.UdpObjDamageId == MyNetworkId)
         {
      ...
      { 
      display ("GetNetworkApiPacket() Packet NOT authenticated. Check NetworkPassword at both ends.", LOG_MOST);
   ...
   if (InPacket.UdpObjCommand == 12)
      { 
      display ("GetNetworkApiPacket() Processing request for administrative details.", LOG_NET);
   ...
   if (InPacket.UdpObjCommand == 13)
      ...
      unsigned char Count;
      display ("GetNetworkApiPacket() Processing Admin013InPacket.", LOG_NET);

      sprintf (DebugBuf, "Admin013InPacket.UdpObjCommunityHandle01 = %s", Admin013InPacket.UdpObjCommunityHandle01);
      display (DebugBuf, LOG_NET);
      strncpy (PlayerIdStrings[0], Admin013InPacket.UdpObjCommunityHandle01, sizeof(PlayerIdStrings[0]));
      ...
      display (DebugBuf, LOG_NET);
      strncpy (PlayerIdStrings[1], Admin013InPacket.UdpObjCommunityHandle02, sizeof(PlayerIdStrings[1]));
      ...
      display (DebugBuf, LOG_NET);
      strncpy (PlayerIdStrings[2], Admin013InPacket.UdpObjCommunityHandle03, sizeof(PlayerIdStrings[2]));
      ...
      display (DebugBuf, LOG_NET);
      strncpy (PlayerIdStrings[3], Admin013InPacket.UdpObjCommunityHandle04, sizeof(PlayerIdStrings[3]));
      ...
      display (DebugBuf, LOG_NET);
      strncpy (PlayerIdStrings[4], Admin013InPacket.UdpObjCommunityHandle05, sizeof(PlayerIdStrings[4]));
      ...
      display (DebugBuf, LOG_NET);
      strncpy (PlayerIdStrings[5], Admin013InPacket.UdpObjCommunityHandle06, sizeof(PlayerIdStrings[5]));
      ...
      display (DebugBuf, LOG_NET);
      strncpy (PlayerIdStrings[6], Admin013InPacket.UdpObjCommunityHandle07, sizeof(PlayerIdStrings[6]));
      ...
      display (DebugBuf, LOG_NET);
      strncpy (PlayerIdStrings[7], Admin013InPacket.UdpObjCommunityHandle08, sizeof(PlayerIdStrings[7]));
      ...
      display (DebugBuf, LOG_NET);
      strncpy (PlayerIdStrings[8], Admin013InPacket.UdpObjCommunityHandle09, sizeof(PlayerIdStrings[8]));
      ...
      display (DebugBuf, LOG_NET);
      strncpy (PlayerIdStrings[9], Admin013InPacket.UdpObjCommunityHandle10, sizeof(PlayerIdStrings[9]));
   ...
   if (InPacket.UdpObjCommand == 14)
      { 
      display ("GetNetworkApiPacket() received UdpObjCommand 14.", LOG_MOST);
      
      memcpy (&Admin014InPacket, ReceiveBuffer, sizeof (Admin014InPacket));
      
      ServerMessageTimer = 0;
      if (Realm == Admin014InPacket.UdpObjRealm && CurrentMissionNumber == Admin014InPacket.UdpObjMissionId)
         { 
         display ("GetNetworkApiPacket() received a SystemMessage for this realm and mission.", LOG_MOST);
...
void SendAdmin012Packet()
{
display ("SendAdmin012Packet()", LOG_NET);
...
if (Admin012BytesSent > 0)
   { 
   AdminOutPacketCount++;
   sprintf (DebugBuf, "SendAdmin012Packet() AdminOutPacketCount = %d, Admin012BytesSent = %d", AdminOutPacketCount, Admin012BytesSent);
   display (DebugBuf, LOG_NET);
   }
else
   {
   display ("SendAdmin012Packet() Couldn't send anything to server.", LOG_MOST);
   }
} 

void SendAdmin013Packet()
{
display ("SendAdmin013Packet()", LOG_NET);
...
if (Admin013BytesSent > 0)
   { 
   AdminOutPacketCount++;
   sprintf (DebugBuf, "SendAdmin013Packet() AdminOutPacketCount = %d, Admin013BytesSent = %d", AdminOutPacketCount, Admin013BytesSent);
   display (DebugBuf, LOG_NET);
   }
else
   {
   display ("SendAdmin013Packet() Couldn't send anything to server.", LOG_MOST);
   }
} 
...
void SendNetworkApiPacket()
{
   ...
   if (MorseCharacterOut)
      { 
      OutPacket.UdpObjTerrainOrMorse = MorseCharacterOut;
      VocalizeMorseCharacter(MorseCharacterOut);
      MorsePanelHighlightTimer = 472;
      unsigned char ScrollPointer = 0;
      ...
      sprintf (DebugBuf, "%c", MorseCharacterOut);
      strncat (MorseRadioMessageBuffer, DebugBuf, 1);
      MorseCharacterOut = 0; 
   ...
   if (OutPacket.UdpObjDamageId == 28)
      ...
      if (OutPacket.UdpObjDamageAmount > 0.0)
         {
         sprintf (DebugBuf, "SendNetworkApi(): You damaged the BlueTeam HQ Airfield by %f kilojoules.", OutPacket.UdpObjDamageAmount);
         display (DebugBuf, LOG_MOST);
         }
      }
   if (OutPacket.UdpObjDamageId == 29)
      ...
      if (OutPacket.UdpObjDamageAmount > 0.0)
         {
         sprintf (DebugBuf, "SendNetworkApi(): You damaged the RedTeam HQ Airfield by %f kilojoules.", OutPacket.UdpObjDamageAmount);
         display (DebugBuf, LOG_MOST);
         }
      }
   ...
   if (GunCameraStartSegmentFlag)
      {
      display ("SendNetworkApiPacket() marking a GunCameraStartSegment.", LOG_MOST);
   ...
   if (GunCameraStopSegmentFlag)
      ...
      display ("SendNetworkApiPacket() marking a GunCameraStopSegment.", LOG_MOST);
      sprintf (DebugBuf, "   Changing OutPacket.UdpObjSpeed from %f", OutPacket.UdpObjSpeed);
      display (DebugBuf, LOG_MOST);
   ...
   if (BytesSent > 0)
      ...
      display ("Data transmitted. BytesSent=", LOG_NET);
      ...
      {
      display ("Data NOT transmitted.", LOG_NET);
      display ("Return Code=", LOG_NET);
      sprintf (DebugBuf, "%i", BytesSent);
      display (DebugBuf, LOG_NET);
      sprintf (DebugBuf, "%s", strerror(errno));
      display (DebugBuf, LOG_NET);
