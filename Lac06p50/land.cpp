...
void Landscape::flatten (int x, int y, int dx, int dy)
    {
    display ("Landscape::flatten()", LOG_MOST);
    int i, i2;
...
void Landscape::genSurface (int hoehepc, int *heightmap)
    ...
    ActiveTerrainFile = fopen ("DefaultHeightMap.LAC", "rb");
    if (ActiveTerrainFile != NULL)
       { 
       display ("Terrain file DefaultHeightMap.LAC found.", LOG_MOST);
       display ("Loading previously active terrain....", LOG_MOST);
       ...
       display ("Loaded heightmap from DefaultHeightMap.LAC", LOG_MOST);
       int i;
       int i2;
       ...
       { 
       display ("Terrain file DefaultHeightMap.LAC not found.", LOG_MOST);
       display ("Generating new terrain....", LOG_MOST);
       }
    type = LAND_ALPINE;
    hoehe = hoehepc * 200; 
    init ();
...
void Landscape::calcWoods (int dy)
    {
    display ("Function Entry calcWoods()", LOG_MOST);
    int i, i2;
    int var = 2000;
...
void Landscape::searchPlain (int divx, int divy, int *x, int *y)
    ...
    if (divx == -1 || divy == -1)
        {
        display ("Landscape::searchPlain() performing global search.", LOG_MOST);
        ...
        for (i = 8; i <= MAXX - 8; i += 8)
            for (i2 = 8; i2 <= MAXX - 8; i2 += 8)
                ...
                if ((f [i] [i2] == GRASS && f [i-4] [i2] == GRASS && f [i+4] [i2] == GRASS && f [i] [i2-4] == GRASS && f [i] [i2+4] == GRASS) ||
                    ...
                    {
                    display ("searchPlain() found GRASS or REDSAND area", LOG_MOST);
                    if (type != 2)
                        {
                        display ("searchPlain() type != 2", LOG_MOST);
        ...
        for (i = 1; i < MAXX_8; i ++)
            for (i2 = 1; i2 < MAXX_8; i2 ++)
                {
                //display ("searchPlain() seeking min", LOG_MOST);
                if (val [i] [i2] < min)
                    {
                    min = val [i] [i2];
                    *x = 8 * i;
