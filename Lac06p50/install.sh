...
echo and reset your existing configuration back to default values.
echo
...
skip7:
    ...
    cp bin/Release/Lac06p50 $EXECUTABLE_DIR/Lac06p50
    echo Copying default terrain...
    cp bin/Release/DefaultHeightMap.LAC $EXECUTABLE_DIR/DefaultHeightMap.LAC
...
echo $CONFIG_DIR/LacControls.txt file. You can change your
echo flight control definitions by editing that file with a
echo simple text editor. You can also change the most common
echo control definitions from prominent menus within the simulator.
echo
echo PRESS ANY KEYBOARD KEY TO CONTINUE....
read -n 1 -s
echo
echo If LINUX AIR COMBAT does not run well on your computer, you
echo can try deleting the LacConfig.txt file now stored in your
echo new, hidden subdirectory at $CONFIG_DIR. That will force
echo automatic regeneration of the main configuration file with
echo more generic default values.
echo 
echo online help is always available at
echo
echo www.AskMisterWizard.com/FlightSimMovies/LAC
echo
